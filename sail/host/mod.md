...
- [Code storage](code.md) — content-addressed code and `JUMPDEST`
  bitmaps.
- [Validation debug (optional)](debug_disabled.md) — stubbed
  diagnostic variants.
- [Ancestor hashes](environment.md) — the `BLOCKHASH` table.
- [The suspended-frame stack](frame_stack.md) — parked caller frames
  across message calls.
- [The state journal](journal.md) — the closed undo-entry algebra and
  its checkpoint, commit, and revert contracts.
