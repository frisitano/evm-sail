...
static void htr_sha256(const uint8_t *bytes, uint64_t len, htr_root out) {
  ...
  memcpy(out, digest.data, sizeof(htr_root));
}
...
typedef int (*htr_item_root_fn)(const uint8_t *, htr_root);

static int htr_fixed_list(const struct htr_span *span, uint64_t item_size,
                          unsigned depth, htr_item_root_fn item_root,
                          htr_root out) {
  ...
  for (uint64_t i = 0; i < count; i++) {
    htr_root item;
    if (!item_root(span->bytes + i * item_size, item) ||
...
static int htr_execution_requests(const struct htr_input_view *input,
                                  htr_root out) {
  htr_root leaves[5];
  return htr_fixed_list(&input->deposits, 192, 13, htr_deposit, leaves[0]) &&
         htr_fixed_list(&input->withdrawal_requests, 76, 4,
                        htr_withdrawal_request, leaves[1]) &&
         htr_fixed_list(&input->consolidation_requests, 116, 1,
                        htr_consolidation_request, leaves[2]) &&
         htr_fixed_list(&input->builder_deposit_requests, 184, 6,
                        htr_builder_deposit_request, leaves[3]) &&
         htr_fixed_list(&input->builder_exit_requests, 68, 4,
                        htr_builder_exit_request, leaves[4]) &&
...
EVMSAIL_HASH_RETURN evmsail_htr_new_payload_request(
  ...
  if (htr_resolve_input(&input, input_ref) &&
    ...
    EVMSAIL_RETURN_HASH_BE_BYTES(result, root);
  }
  EVMSAIL_RETURN_HASH_BE_BYTES(result, root);
}
