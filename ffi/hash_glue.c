...
/* The generated ByteSource enum follows its declaration order in
 * primitives/bytes.sail, which byte_source_kind numbers 1-based -- one
 * contract, so the kind is the enum value + 1. */
static uint64_t seg_source_kind(enum zByteSource s) { return (uint64_t)s + 1; }
