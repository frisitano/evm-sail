...
static void arena_reserve(size_t need) {
  ...
  if (need > evmsail_capacity_limit(EVMSAIL_CAP_MEMORY_BYTES) ||
      need > arena_cap)
    abort();
  return;
#endif
...
static uint64_t mem_frame_enter_value(void) {
  ...
  return arena ? (uint64_t)(uintptr_t)(arena + f_base[h_top]) : 0;
#else
...
uint64_t evm_memory_expand(uint64_t len) {
  f_establish(len);
#ifdef EVMSAIL_POINTER_ABI
  return len ? (uint64_t)(uintptr_t)(arena + f_base[h_top]) : 0;
#else
  return (uint64_t)f_base[h_top];
#endif
}
...
const uint8_t *evm_memory_region(uint64_t off, uint64_t len) {
  ...
  if (!arena || pointer < base || pointer - base > arena_cap ||
      len > arena_cap - (size_t)(pointer - base))
    return NULL;
