 ...
 * execution lookup does not hash. State-root iteration is a separate sorted view
 * ordered by keccak256(address), the secure trie key that Sail passes on each
 * store. The witness-authenticated pre-state is the base it sits over: a miss in
 * stateless mode is materialized once from the acctdb span store (one keccak)
 * and cached here by raw address.
 *
 ...
 * selected once with acctmap_key (caches addr + raw lookup hash) and the
 * accessors resolve against it. Iteration (acctmap_count/_at/_at_*) feeds dumps
 * and post-state root updates in secure-key order. */
