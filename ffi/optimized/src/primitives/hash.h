...
static inline int hash_compare(const Hash32 *a, const Hash32 *b)
{
  return memcmp(a->bytes, b->bytes, sizeof(a->bytes));
}

static inline int hash_equal(const Hash32 *a, const Hash32 *b)
{
  return memcmp(a->bytes, b->bytes, sizeof(a->bytes)) == 0;
}

/* The low 160 bits of the digest, retaining canonical Ethereum byte order. */
static inline Address hash_low_address(const Hash32 *value)
{
  Address result;
