...
static bool mpt_encode_leaf_reference(const NibblePath *path,
                                      ...
                                      NodeReference *out) {
  uint8_t compact[33], encoded[MPT_ENCODED_NODE_MAX];
...
static bool mpt_next_storage_update(MptUpdateSource *source,
  ...
  *found = false;
  while (source->position < source->count) {
    ...
    if (!storage_trie_binding_get(
            mpt_storage_update_id_at(source->position++),
            source->storage_generation, &view))
      return false;
    memset(update, 0, sizeof(*update));
    update->key = nibble_path_from_secure_key(view.secure_key);
    update->changed =
        memcmp(view.current, view.original, sizeof(*view.current)) != 0;
    if (!update->changed)
      continue;
    update->delete = mpt_word_zero(*view.current);
    if (!update->delete &&
        !mpt_encode_storage(*view.current, update->value,
                            &update->value_len))
      return false;
    ...
    if (update->original_present &&
        !mpt_encode_storage(*view.original, update->original,
                            &update->original_len))
      return false;
    *found = true;
    return true;
...
static bool
mpt_make_account_update(const AccountTrieView *meta,
                        const Hash32 *storage_root,
                        TrieUpdate *update) {
  ...
  if (update->changed && !update->delete) {
    if (!mpt_encode_account(meta->nonce, *meta->balance, storage_root,
                            meta->code_hash,
...
static bool mpt_order_less(MptOrderKind kind, const TrieUpdateBuffer *updates,
  ...
  case MPT_ORDER_STORAGE_BINDINGS:
    if (!storage_trie_binding_order_key(
            left, order_workspace.storage_generation, &left_node,
            &left_key) ||
        !storage_trie_binding_order_key(
            right, order_workspace.storage_generation, &right_node,
            &right_key))
      GUEST_ABORT();
    break;
...
uint32_t mpt_storage_updates_prepare(AccountId account_id,
  ...
  StorageId begin;
  const uint32_t candidates = storage_trie_candidates(
      account_id, &begin, &order_workspace.storage_generation);
  *generation = order_workspace.storage_generation;
  ...
  for (StorageId id = begin; id < begin + candidates; ++id) {
    ...
    Hash32 secure_key;
    if (storage_trie_binding_order_key(
            id, order_workspace.storage_generation, &terminal_node,
            &secure_key))
