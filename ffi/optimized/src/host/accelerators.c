...
void accelerators_workspace_bind(void) {
  WORKSPACE_BIND(input_scratch, GUEST_ACCELERATOR_INPUT_BYTES);
  WORKSPACE_BIND(bls_scratch, GUEST_ACCELERATOR_WORK_BYTES);
}

static uint8_t *input_workspace(uint64_t need) {
  return need <= GUEST_ACCELERATOR_INPUT_BYTES ? input_scratch : NULL;
}
...
static uint8_t *materialize(const AcceleratorInput *input, uint64_t padded_len) {
  uint8_t *bytes = input_workspace(padded_len ? padded_len : 1);
...
static uint8_t *reserve_bls(uint64_t count, uint64_t item_size) {
  ...
  return count * item_size <= GUEST_ACCELERATOR_WORK_BYTES ? bls_scratch
                                                            : NULL;
}
