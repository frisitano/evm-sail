...
static char validation_location[513];

unit validation_debug_capture_location(unit ignored)
{
    (void)ignored;
    const char *location = exception_location ? exception_location : "";
    size_t len = 0;
    while (location[len] && len < sizeof validation_location - 1) {
        validation_location[len] = location[len];
        len++;
    }
    validation_location[len] = '\0';
    return UNIT;
}

static void append_byte(unsigned char byte)
{
    if (dump_len < sizeof dump_bytes) dump_bytes[dump_len++] = byte;
}
...
{
    size_t len = 0;
    while (location[len] && len < 512) len++;
...
static uint32_t account_count(uint32_t binding_count)
{
    ...
    for (uint32_t index = 0; index < binding_count; index++) {
        ...
        if (account_trie_binding_get(index, &view) &&
            account_exists(view.account_id))
            count++;
    }
    return count;
}
...
static void append_storage(AccountId account_id)
{
    ...
    const uint32_t candidates =
        storage_trie_candidates(account_id, &begin, &generation);
    uint32_t count = 0;
...
static void append_accounts(bool state_available)
{
    const uint32_t binding_count =
        state_available ? mpt_account_updates_prepare() : 0;
    ...
    for (uint32_t index = 0; index < binding_count; index++) {
        ...
        const Hash32 storage_root =
            mpt_storage_root_hash(view.original_storage_root_node);
...
unsigned long guest_debug_dump(const unsigned char **out)
{
    ...
    if (state_available) {
        ...
        for (size_t index = 0; index < 32; index++) append_byte(0);
        append_byte(have_exception
                        ? (unsigned char)current_exception.variants.InvalidBlock
                        : validation_failed
                              ? (unsigned char)validation_failure_reason
                              : 0xff);
        append_location(have_exception && exception_location
                            ? exception_location
                            : "");
    }
    ...
    if (validation_failure_present) {
        ...
        append_location(validation_location);
    }
    ...
    append_byte('S');
    const uint16_t depth = state_available ? stack_depth(UNIT) : 0;
    append_u32(depth);
    for (uint16_t index = 0; index < depth; index++)
        append_word(stack_peek_word(index));

    append_byte('M');
    append_u32(state_available ? (uint32_t)hm_depth(UNIT) : 0);
