...
extern unit environment_reset(unit);         /* fixed BLOCKHASH ancestor table */
                                 ...
                                 unsigned line)
{
    (void)snprintf(last_error, sizeof last_error,
                   "optimized FFI invariant failed at %s:%u in %s()",
                   file ? file : "?", line, function ? function : "?");
    if (panic_boundary != NULL) longjmp(*panic_boundary, 1);
    abort();
}
...
static void *run_thread(void *unused)
{
    ...
    if (setjmp(boundary) == 0) (void)zmain(UNIT);
    panic_boundary = NULL;
    return NULL;
}
