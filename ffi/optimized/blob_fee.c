 ...
 * integers. Optimized C evaluates the same recurrence in a six-limb
 * accumulator. It stops only when the exact result cannot fit the model's
 * EVM-word return type: below that boundary, the largest intermediate
 * multiplication is less than 2^320 and therefore fits the 384-bit
 * accumulator.
 */
#include "blob_fee.h"
#include "exception.h"
...
static struct blob_fee_observation blob_base_fee_above_word(void) {
  return (struct blob_fee_observation){
      .value = (sail_u256){{0, 0, 0, 0}},
      .exceeds_word = true,
  };
}

static struct blob_fee_observation blob_base_fee_observe(
    uint64_t excess_blob_gas) {
  const uint64_t denominator = zk_blob_schedule.zbase_fee_update_fraction;
  struct blob_big output = {{0}};
  struct blob_big term = {{denominator, 0, 0, 0, 0, 0}};
  /* Scaled output at which output / denominator no longer fits u256. */
  struct blob_big output_limit = {{0, 0, 0, 0, denominator, 0}};
  uint64_t iteration = 1;

  while (!blob_big_is_zero(&term)) {
    if (!blob_big_add(&output, &term) ||
        blob_big_compare(&output, &output_limit) >= 0) {
      return blob_base_fee_above_word();
    }
  ...
  if ((fee.limbs[4] | fee.limbs[5]) != 0) {
    return blob_base_fee_above_word();
  }
  return (struct blob_fee_observation){
      .value =
          (sail_u256){
              {fee.limbs[0], fee.limbs[1], fee.limbs[2], fee.limbs[3]}},
      .exceeds_word = false,
  };
}

sail_u256 evmsail_blob_base_fee_word(uint64_t excess_blob_gas) {
  const struct blob_fee_observation observation =
      blob_base_fee_observe(excess_blob_gas);
  if (observation.exceeds_word) {
    evmsail_throw_invalid_block(zExecutionInvalid, "blob_base_fee");
  }
  return observation.value;
}

static bool blob_fee_times_16_lt_base(sail_u256 fee, sail_u256 base_fee) {
  if ((fee.limbs[3] >> 60) != 0) return false;

  const sail_u256 scaled = {{
      fee.limbs[0] << 4,
      (fee.limbs[1] << 4) | (fee.limbs[0] >> 60),
      (fee.limbs[2] << 4) | (fee.limbs[1] >> 60),
      (fee.limbs[3] << 4) | (fee.limbs[2] >> 60),
  }};
  for (size_t i = 4; i-- > 0;) {
    if (scaled.limbs[i] < base_fee.limbs[i]) return true;
    if (scaled.limbs[i] > base_fee.limbs[i]) return false;
  }
  return false;
}
...
bool evmsail_blob_reserve_price_for_excess(
  ...
  const struct blob_fee_observation observation =
      blob_base_fee_observe(excess_blob_gas);
  return !observation.exceeds_word &&
         blob_fee_times_16_lt_base(observation.value, base_fee);
}
