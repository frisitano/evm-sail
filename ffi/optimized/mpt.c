...
static bool sr_hash_equal(const sail_fixed_bytes_32 *a, const uint8_t b[32]) {
  return memcmp(a->bytes, b, 32) == 0;
}

static bool sr_word_zero(sail_u256 value) {
  return (value.limbs[0] | value.limbs[1] | value.limbs[2] |
          value.limbs[3]) == 0;
}

static bool sr_word_equal(sail_u256 a, sail_u256 b) {
...
static struct sr_path sr_path_hash(const sail_fixed_bytes_32 *hash) {
  struct sr_path path;
  path.len = 64;
...
static bool sr_lookup_root(struct sr_ctx *ctx, const sail_fixed_bytes_32 *root,
                           struct sr_span *out) {
  struct sr_ref ref = sr_hash_ref(root->bytes);
  return sr_resolve_ref(ctx, &ref, out);
}

static uint8_t sr_hash_nibble(const sail_fixed_bytes_32 *key, unsigned position) {
  const uint8_t byte = key->bytes[position / 2];
  return (position & 1) ? byte & 0x0f : byte >> 4;
}

static bool sr_key_matches(const sail_fixed_bytes_32 *key, unsigned position,
                           const struct sr_path *path) {
 ...
 */
static bool sr_point_lookup(struct sr_ctx *ctx, const sail_fixed_bytes_32 *root,
                            const sail_fixed_bytes_32 *key, struct sr_span *value) {
  value->data = NULL;
  value->len = 0;
...
static bool sr_decode_hash_item(struct sr_ctx *ctx,
                                const struct sr_rlp_item *item,
                                const uint8_t empty[32], sail_fixed_bytes_32 *value) {
...
static size_t sr_rlp_word_size(sail_u256 value) {
  uint8_t bytes[32];
  sail_word_to_be_bytes(bytes, value);
  size_t first = 0;
  while (first < 32 && bytes[first] == 0) ++first;
...
static bool sr_encode_storage(struct sr_ctx *ctx, sail_u256 value,
...
static bool sr_encode_account(struct sr_ctx *ctx, uint64_t nonce,
                              sail_u256 balance, const sail_fixed_bytes_32 *storage_root,
                              const sail_fixed_bytes_32 *code_hash,
                              uint8_t out[SR_VALUE_MAX], uint8_t *out_len) {
...
static bool sr_storage_next(struct sr_ctx *ctx, struct sr_source *source,
  ...
  sail_u256 slot, current, original;
  sail_fixed_bytes_32 address_hash, slot_hash;
  *found = false;
...
static bool sr_account_empty(uint64_t nonce, sail_u256 balance,
                             const sail_fixed_bytes_32 *code_hash) {
...
static bool sr_account_fields_changed(
    uint64_t current_nonce, sail_u256 current_balance,
    const sail_fixed_bytes_32 *current_storage, const sail_fixed_bytes_32 *current_code,
    bool current_exists, bool current_storage_cleared, uint64_t original_nonce,
    sail_u256 original_balance, const sail_fixed_bytes_32 *original_storage,
    const sail_fixed_bytes_32 *original_code, bool original_exists,
...
static bool sr_account_next(struct sr_ctx *ctx, struct sr_source *source,
  ...
  *found = false;
  while (source->position < source->count) {
    ...
    uint64_t cn = 0, on = 0;
    sail_u256 cb = {{0}}, ob = {{0}};
    sail_fixed_bytes_32 cs = {{0}}, cc = {{0}}, os = {{0}}, oc = {{0}};
    sail_fixed_bytes_32 address_hash = {{0}}, storage_root = {{0}};
...
static bool sr_trie_root(struct sr_ctx *ctx, const sail_fixed_bytes_32 *base_root,
                         struct sr_source source, sail_fixed_bytes_32 *root,
                         bool *changed) {
  struct sr_cursor cursor;
...
static bool sr_prepare_storage_roots(struct sr_ctx *ctx,
  ...
  for (uint32_t account_index = 0; account_index < account_count;
    ...
    sail_u256 cb = {{0}}, ob = {{0}};
    sail_fixed_bytes_32 cs = {{0}}, cc = {{0}}, os = {{0}}, oc = {{0}};
    sail_fixed_bytes_32 address_hash = {{0}}, ignored_post_storage_root = {{0}};
    bool ce = false, csc = false, ccr = false, csd = false;
    bool oe = false, osc = false, ocr = false, osd = false;
    if (!acct_block_update_probe_at(
    ...
    sail_fixed_bytes_32 base = cs;
    if (csc) memcpy(base.bytes, sr_empty_trie_root, 32);
    sail_fixed_bytes_32 post = base;
...
static void sr_ordered_root_finish(struct sr_ctx *ctx, sail_fixed_bytes_32 *root) {
  if (ctx->status == SR_OK &&
