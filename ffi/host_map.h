/* C-backed layered (address, slot) -> 256-bit value maps (see host_map.c):
...
#include "sail.h"
#include <stdint.h>
unit host_map_reset(uint64_t id);                       /* clear to one empty base layer */
unit host_map_push(uint64_t id);                        /* open an overlay (frame/tx)    */
unit host_map_pop_commit(uint64_t id);                  /* merge top into the layer below */
unit host_map_pop_discard(uint64_t id);                 /* drop top (revert)             */
...
unit host_map_store(uint64_t v3, uint64_t v2, uint64_t v1, uint64_t v0); /* write (top) */
...
unit host_map_sel(uint64_t j);                          /* select flattened entry j       */
uint64_t host_map_it_key(uint64_t i);                   /* key word: 0..2 addr, 3..6 slot */
uint64_t host_map_it_val(uint64_t i);                   /* value word (3=msw..0)          */
#endif
