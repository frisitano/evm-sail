...
static const uint8_t *require_region_pointer(const char *region,
                                             const uint8_t *pointer,
                                             uint64_t off, uint64_t len) {
  if (pointer) return pointer;
  fprintf(stderr, "evmsail: invalid %s pointer %#llx+%llu\n", region,
...
const uint8_t *evmsail_stateless_input_ptr(uint64_t off, uint64_t len) {
  ...
  if (!private_input || pointer < base ||
      pointer - base > private_input_size ||
      len > private_input_size - (size_t)(pointer - base)) {
    fprintf(stderr,
            "evmsail: invalid stateless-input pointer %#llx+%llu "
            "(base=%p len=%zu)\n",
            (unsigned long long)off, (unsigned long long)len,
            (const void *)private_input, private_input_size);
    abort();
  }
...
#ifdef EVMSAIL_POINTER_ABI
  return require_region_pointer("memory", bytes, off, len);
#else
  return bytes;
#endif
}
...
const uint8_t *evmsail_code_ptr(uint64_t off, uint64_t len) {
  ...
  if (!code_db_resolve_code(off, len, &bytes, &resolved_len) ||
      resolved_len != len) {
#ifdef EVMSAIL_POINTER_ABI
    return require_region_pointer("code", NULL, off, len);
#else
    return NULL;
#endif
  }
...
#ifdef EVMSAIL_POINTER_ABI
  return require_region_pointer("scratch", bytes, off, len);
#else
  return bytes;
#endif
}
...
#ifdef EVMSAIL_POINTER_ABI
  return require_region_pointer("log-data", bytes, off, len);
#else
  return bytes;
#endif
}
...
const uint8_t *evmsail_output_ptr(uint64_t off, uint64_t len) {
...
#ifdef EVMSAIL_POINTER_ABI
    return require_region_pointer("output", NULL, off, len);
#else
    return NULL;
#endif
  }
  ...
  uintptr_t pointer = (uintptr_t)off;
  if (!bytes || pointer < base || pointer - base > total ||
      len > total - (uint64_t)(pointer - base))
    return require_region_pointer("output", NULL, off, len);
  return (const uint8_t *)pointer;
#else
  if (off > total || len > total - off) return NULL;
...
static void full_input_value(struct zStatelessInputSliceFields *out) {
  ...
  stateless_input_value(
      out,
#ifdef EVMSAIL_POINTER_ABI
      private_input_size ? (uint64_t)(uintptr_t)private_input : 0,
#else
      0,
#endif
...
struct zOutputSliceFields evmsail_output_buffer_slice(uint64_t len) {
  ...
  const uint8_t *bytes = NULL;
  uint64_t total = 0;
  if (!output_buffer_span(&bytes, &total) || len > total) abort();
#ifdef EVMSAIL_POINTER_ABI
  evmsail_byte_quantity_set(&out.zoff,
                            len ? (uint64_t)(uintptr_t)bytes : 0);
#else
...
static sail_word region_load_word(span_resolver resolve, uint64_t off,
                                  uint64_t len, uint64_t index) {
  uint8_t bytes[32] = {0};
  if (index < len) {
...
static sail_word region_load_n_word(span_resolver resolve, uint64_t off,
                                    uint64_t len, uint64_t index,
                                    uint64_t requested) {
  uint8_t bytes[32] = {0};
...
#define DEFINE_SLICE_LOAD(prefix, type, resolver)                             \
  EVMSAIL_WORD_RETURN prefix##_load_word(                                     \
      ...
      EVMSAIL_BYTE_QUANTITY_PARAM(index)) {                                    \
    EVMSAIL_RETURN_WORD(                                                       \
        result, region_load_word(resolver, SLICE_OFF(slice), SLICE_LEN(slice), \
                                 evmsail_byte_quantity_value(index)));          \
  }
...
#define DEFINE_SLICE_LOAD_N(prefix, type, resolver)                           \
  EVMSAIL_WORD_RETURN prefix##_load_n_word(                                   \
    ...
    EVMSAIL_RETURN_WORD(                                                       \
        result,                                                               \
        region_load_n_word(resolver, SLICE_OFF(slice), SLICE_LEN(slice),       \
                           evmsail_byte_quantity_value(index),                  \
...
#ifdef EVMSAIL_POINTER_ABI
      end ? (uint64_t)(uintptr_t)scratch_base() : 0,
#else
      0,
#endif
      end);
...
void scratch_store_word(struct zScratchRegionResult *result,
  ...
  for (uint64_t i = 0; i < len; ++i) {
    ...
    out[i] = (uint8_t)(EVMSAIL_WORD_VALUE(data).limbs[byte_from_low / 8] >>
                       (8 * (byte_from_low % 8)));
  }
  const bool accepted = scratch_commit(dst, len);
  scratch_result_value(result, accepted, accepted ? dst + len : 0);
...
EVMSAIL_HASH_RETURN code_db_store_indexed(
  ...
  return code_db_store_indexed_bytes(
      EVMSAIL_HASH_RESULT(result)
          evmsail_code_ptr(SLICE_OFF(code), SLICE_LEN(code)),
      SLICE_LEN(code), jumpdest_ref);
}
