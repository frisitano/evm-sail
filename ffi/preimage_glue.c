...
static sail_hash zero_hash(void) {
  sail_hash result = {{0}};
  return result;
}

static sail_hash keccak_bytes(const uint8_t *bytes, uint64_t len) {
...
static sail_hash sha256_bytes(const uint8_t *bytes, uint64_t len) {
...
static unsigned u256_width(sail_word value) {
  uint8_t bytes[32];
...
static uint64_t rlp_quantity_size_u256(sail_word value) {
...
static uint8_t *write_rlp_u256(uint8_t *out, sail_word value) {
  uint8_t bytes[32];
  sail_word_to_be_bytes(bytes, value);
  const unsigned width = u256_width(value);
  if (width == 0) {
...
static bool u256_gte_small(sail_word value, uint64_t small) {
  return value.limbs[1] != 0 || value.limbs[2] != 0 ||
         value.limbs[3] != 0 || value.limbs[0] >= small;
}

static sail_word legacy_chain_id(sail_word v) {
  sail_word result = v;
...
EVMSAIL_ADDRESS_RETURN evmsail_optimized_create2_address(
  ...
  if (!preimage) {
    sail_address zero = {{0}};
    EVMSAIL_RETURN_ADDRESS(result, zero);
  }
  preimage[0] = 0xff;
  ...
  const sail_hash digest = keccak_bytes(preimage, 85);
  sail_address address;
  memcpy(address.bytes, digest.bytes + 12, sizeof(address.bytes));
  EVMSAIL_RETURN_ADDRESS(result, address);
}
...
EVMSAIL_HASH_RETURN evmsail_optimized_tx_signing_hash(
  ...
  const sail_word zero_word = {{0}};
  const sail_word chain_id = eip155 ? legacy_chain_id(v) : zero_word;
  ...
  if (type > 4 || content_len > (UINT64_C(1) << 30) ||
    ...
    EVMSAIL_RETURN_HASH(result, zero_hash());
  ...
  if ((uint64_t)(cursor - preimage) != total)
    EVMSAIL_RETURN_HASH(result, zero_hash());
  EVMSAIL_RETURN_HASH(result, keccak_bytes(preimage, total));
}
...
EVMSAIL_HASH_RETURN evmsail_optimized_sha256_request_digest(
  ...
  EVMSAIL_RETURN_HASH(result, sha256_bytes(preimage, request_len + 1));
}
...
EVMSAIL_HASH_RETURN evmsail_optimized_execution_requests_hash(
  ...
  if (request_digest(4, input_ref.zbuilder_exit_requests, &digests[count]))
    ++count;
  EVMSAIL_RETURN_HASH(
      result, sha256_bytes((const uint8_t *)digests, count * sizeof(*digests)));
}
