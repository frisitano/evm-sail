...
static code_db_ent *code_db_cur;  /* entry selected by cs_begin          */
...
/* copy the witness code for codeHash (h3,h2,h1,h0) from the SSZ input into the
 * code_db. Content-addressed: a hash already present is left as-is (dedup). */
unit cs_view_hash_input(uint64_t h3, uint64_t h2, uint64_t h1, uint64_t h0,
  ...
  for (uint32_t i = 0; i < e->len; i++) {
    uint64_t p = off + i;
    e->p[i] = (p < off) ? 0 : (uint8_t)ssz_src_byte_u64(p);
  }
  e->bm = (uint8_t *)malloc(e->len / 8 + 1);
  fc_build_bitmap_bytes(e->bm, e->p, e->len);
  return UNIT;
}
...
static const code_db_ent *cs_get(uint64_t h3, uint64_t h2, uint64_t h1, uint64_t h0) {
...
uint64_t cs_len(uint64_t h3, uint64_t h2, uint64_t h1, uint64_t h0) {
  const code_db_ent *e = cs_get(h3, h2, h1, h0);
  return e ? e->len : 0;
}
