...
# and the zkVM guest via the EVM_BACKEND=build project selection.
# ===========================================================================
...
# columns line up.
lint:
	@o=$$(for e in $(WARN_ENTRIES); do $(SAIL) --all-warnings $(PROJECT) evm --variable EVM_BACKEND=spec --variable EVM_ENTRY="$$e" 2>&1; done); if printf '%s\n' "$$o" | grep -qiE "warning|error"; then printf '%s\n' "$$o" | grep -iE "warning|error" | head -20; echo "lint: FAILED (sail warnings)"; exit 1; fi; \
...
# sail --html can't take the project module directly, so concatenate the core
# entry's files in evm.sail_project order into one source and render that.
html:
	@order=$$($(SAIL) --project $(PROJECT) --list-files evm --variable EVM_BACKEND=spec --variable EVM_ENTRY=core); \
