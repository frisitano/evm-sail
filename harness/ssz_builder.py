...
def _header(fk, **ov):
    ...
    return fk.Header(
        **{name: value for name, value in d.items() if name in header_fields}
    )
...
def _ancestor_headers(
    fk, pre_root, number, parent_excess, parent_blob_used, base_fee, gas_limit
):
...
def _serialize_v062_pair(stateless_input, successful_validation, fork_name):
    from v062_compat import serialize_v062_pair
...
def _build_state_test_guest(case):
    ...
    header_fields = {field.name for field in dataclasses.fields(fk.Header)}
    chain_id = _hi(
        case.get("config", {}).get("chainid", env.get("currentChainId", "0x1"))
    )
    ...
    txs_input = (
        _t8n_rlp_transactions(tx_hex) if tx_hex else [_t8n_tx(tx, idx, chain_id)]
    )
    stdin_json = {
        "alloc": pre,
        "env": t8n_env,
