...
from devtools.optimised_c.build import ROOT
...
def resolved(args: argparse.Namespace) -> tuple[Path, Path, Path, dict[str, Any]]:
    return resolve_compiler_provenance(args.sail, args.sail_source, args.sail_effective_binary)
...
def run_extraction(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(
        ...
        snapshot_plugins = snapshot_root / "_build/install/default/share/libsail/plugins"
        ...
        if (
            snapshot_plugin_identity["plugin_tree_sha256"] != before["plugin_tree_sha256"]
            or snapshot_plugin_identity["plugin_file_count"] != before["plugin_file_count"]
        ):
        ...
        if (
            snapshot_library_identity["library_tree_sha256"] != before["library_tree_sha256"]
            or snapshot_library_identity["library_file_count"] != before["library_file_count"]
        ):
        ...
        result = subprocess.run(command, cwd=args.cwd.resolve(), env=environment, check=False)
