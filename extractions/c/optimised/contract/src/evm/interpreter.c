...
static inline const uint8_t *interpreter_code_bytes(const struct InterpreterFrameContext *frame)
{
  return frame->code.len == 0 ? &empty_code_byte : frame->code.bytes;
}

static inline uint32_t interpreter_cursor_pc(const struct InterpreterFrameContext *frame,
                                             const uint8_t *ip)
{
  return (uint32_t)(ip - interpreter_code_bytes(frame));
}

static inline void interpreter_load_cursor(const struct InterpreterFrameContext *frame,
                                           const uint8_t **ip, const uint8_t **code_end)
{
  const uint8_t *code_bytes = interpreter_code_bytes(frame);
  const uint32_t pc = frame->next_pc < frame->code.len ? frame->next_pc : frame->code.len;
  *ip = code_bytes + pc;
...
#define EXECUTE_HANDLER(call)                                                                      \
  do {                                                                                             \
    struct tuple_uint_64_StackPointer_OpcodeOutcome result = (call);                               \
    gas = result.tup0;                                                                             \
    sp = result.tup1;                                                                              \
    FINISH_OPCODE(result.tup2);                                                                    \
  } while (0)

#define EXECUTE_MEMORY_HANDLER(call)                                                               \
  do {                                                                                             \
    struct tuple_uint_64_StackPointer_Bytes_OpcodeOutcome result = (call);                         \
    gas = result.tup0;                                                                             \
    sp = result.tup1;                                                                              \
    memory = result.tup2;                                                                          \
    FINISH_OPCODE(result.tup3);                                                                    \
  } while (0)
                   ...
                   struct CalldataSlice initial_calldata, uint8_t fork, u256 blob_fee)
{
  static const void *const dispatch[256] = {
      ...
      [0x46] = &&opcode_chainid,
      [0x47] = &&opcode_selfbalance,
      [0x48] = &&opcode_basefee,
      [0x49] = &&opcode_blobhash,
      [0x4a] = &&opcode_blobbasefee,
      [0x4b] = &&opcode_slotnum,
      [0x4c ... 0x4f] = &&opcode_invalid,
      [0x50] = &&opcode_pop,
      [0x51] = &&opcode_mload,
      [0x52] = &&opcode_mstore,
      [0x53] = &&opcode_mstore8,
      ...
      [0xf0] = &&opcode_frame_entry,
      [0xf1] = &&opcode_frame_entry,
      [0xf2] = &&opcode_frame_entry,
      [0xf3] = &&opcode_return,
      [0xf4] = &&opcode_frame_entry,
      [0xf5] = &&opcode_frame_entry,
      [0xf6 ... 0xf9] = &&opcode_invalid,
      [0xfa] = &&opcode_frame_entry,
      [0xfb ... 0xfc] = &&opcode_invalid,
      [0xfd] = &&opcode_revert,
      [0xfe] = &&opcode_invalid,
...
opcode_stop:
  ...
  goto interpreter_continue;
opcode_add:
  EXECUTE_HANDLER(execute_add(gas, sp));
opcode_mul:
  EXECUTE_HANDLER(execute_mul(gas, sp));
opcode_sub:
  EXECUTE_HANDLER(execute_sub(gas, sp));
opcode_div:
  EXECUTE_HANDLER(execute_div(gas, sp));
opcode_sdiv:
  EXECUTE_HANDLER(execute_sdiv(gas, sp));
opcode_mod:
  EXECUTE_HANDLER(execute_mod(gas, sp));
opcode_smod:
  EXECUTE_HANDLER(execute_smod(gas, sp));
opcode_addmod:
  EXECUTE_HANDLER(execute_addmod(gas, sp));
opcode_mulmod:
  EXECUTE_HANDLER(execute_mulmod(gas, sp));
opcode_exp:
  EXECUTE_HANDLER(execute_exp(gas, sp));
opcode_signextend:
  EXECUTE_HANDLER(execute_signextend(gas, sp));
opcode_lt:
  EXECUTE_HANDLER(execute_lt(gas, sp));
opcode_gt:
  EXECUTE_HANDLER(execute_gt(gas, sp));
opcode_slt:
  EXECUTE_HANDLER(execute_slt(gas, sp));
opcode_sgt:
  EXECUTE_HANDLER(execute_sgt(gas, sp));
opcode_eq:
  EXECUTE_HANDLER(execute_eq(gas, sp));
opcode_iszero:
  EXECUTE_HANDLER(execute_iszero(gas, sp));
opcode_and:
  EXECUTE_HANDLER(execute_and(gas, sp));
opcode_or:
  EXECUTE_HANDLER(execute_or(gas, sp));
opcode_xor:
  EXECUTE_HANDLER(execute_xor(gas, sp));
opcode_not:
  EXECUTE_HANDLER(execute_not(gas, sp));
opcode_byte:
  EXECUTE_HANDLER(execute_byte(gas, sp));
opcode_shl:
  EXECUTE_HANDLER(execute_shl(gas, sp));
opcode_shr:
  EXECUTE_HANDLER(execute_shr(gas, sp));
opcode_sar:
  EXECUTE_HANDLER(execute_sar(gas, sp));
opcode_clz:
  REQUIRE_OPCODE_AVAILABLE(0x1e);
  EXECUTE_HANDLER(execute_clz(gas, sp));
opcode_keccak256:
  EXECUTE_MEMORY_HANDLER(execute_keccak256(gas, sp, memory));
opcode_address:
  EXECUTE_HANDLER(execute_address(frame.address, gas, sp));
opcode_balance:
  EXECUTE_HANDLER(execute_balance(gas, sp));
opcode_origin:
  EXECUTE_HANDLER(execute_origin(gas, sp));
opcode_caller:
  EXECUTE_HANDLER(execute_caller(frame.caller, gas, sp));
opcode_callvalue:
  EXECUTE_HANDLER(execute_callvalue(frame.value, gas, sp));
opcode_calldataload:
  EXECUTE_HANDLER(execute_calldataload(frame.calldata, gas, sp));
opcode_calldatasize:
  EXECUTE_HANDLER(execute_calldatasize(frame.calldata, gas, sp));
opcode_calldatacopy:
  EXECUTE_MEMORY_HANDLER(execute_calldatacopy(frame.calldata, gas, sp, memory));
opcode_codesize:
  EXECUTE_HANDLER(execute_codesize(frame.code, gas, sp));
opcode_codecopy:
  EXECUTE_MEMORY_HANDLER(execute_codecopy(frame.code, gas, sp, memory));
opcode_gasprice:
  EXECUTE_HANDLER(execute_gasprice(gas, sp));
opcode_extcodesize:
  EXECUTE_HANDLER(execute_extcodesize(gas, sp));
opcode_extcodecopy:
  EXECUTE_MEMORY_HANDLER(execute_extcodecopy(gas, sp, memory));
opcode_returndatasize:
  EXECUTE_HANDLER(execute_returndatasize(frame.returndata, gas, sp));
opcode_returndatacopy:
  EXECUTE_MEMORY_HANDLER(execute_returndatacopy(frame.returndata, gas, sp, memory));
opcode_extcodehash:
  EXECUTE_HANDLER(execute_extcodehash(gas, sp));
opcode_blockhash:
  EXECUTE_HANDLER(execute_blockhash(gas, sp));
opcode_coinbase:
  EXECUTE_HANDLER(execute_coinbase(gas, sp));
opcode_timestamp:
  EXECUTE_HANDLER(execute_timestamp(gas, sp));
opcode_number:
  EXECUTE_HANDLER(execute_number(gas, sp));
opcode_prevrandao:
  EXECUTE_HANDLER(execute_prevrandao(gas, sp));
opcode_gaslimit:
  EXECUTE_HANDLER(execute_gaslimit(gas, sp));
opcode_chainid:
  EXECUTE_HANDLER(execute_chainid(gas, sp));
opcode_selfbalance:
  EXECUTE_HANDLER(execute_selfbalance(frame.address, gas, sp));
opcode_basefee:
...
opcode_pop:
  ...
  NEXT_OPCODE();
opcode_mload:
  EXECUTE_MEMORY_HANDLER(execute_mload(gas, sp, memory));
opcode_mstore:
  EXECUTE_MEMORY_HANDLER(execute_mstore(gas, sp, memory));
opcode_mstore8:
  EXECUTE_MEMORY_HANDLER(execute_mstore8(gas, sp, memory));
opcode_sload:
  EXECUTE_HANDLER(execute_sload(frame.account, gas, sp));
opcode_sstore: {
  struct tuple_uint_64_uint_64_uint_32_int_128_StackPointer_OpcodeOutcome result =
      execute_sstore(frame.account, fork, frame.is_static, gas, state_gas_remaining,
                     state_gas_spilled, refund, sp);
  gas = result.tup0;
  state_gas_remaining = result.tup1;
  state_gas_spilled = result.tup2;
  refund = result.tup3;
  sp = result.tup4;
  FINISH_OPCODE(result.tup5);
}
opcode_jump: {
  frame.next_pc = interpreter_cursor_pc(&frame, ip);
  struct tuple_uint_32_uint_64_StackPointer_OpcodeOutcome result =
...
opcode_pc: {
  frame.next_pc = interpreter_cursor_pc(&frame, ip);
  struct tuple_uint_32_uint_64_StackPointer_OpcodeOutcome result =
      execute_pc(frame.next_pc, gas, sp);
  frame.next_pc = result.tup0;
  gas = result.tup1;
  sp = result.tup2;
  interpreter_load_cursor(&frame, &ip, &code_end);
...
opcode_msize:
  EXECUTE_MEMORY_HANDLER(execute_msize(gas, sp, memory));
opcode_gas:
...
opcode_mcopy:
  ...
  EXECUTE_MEMORY_HANDLER(execute_mcopy(gas, sp, memory));
...
opcode_log_family: {
  struct tuple_uint_64_StackPointer_Bytes_OpcodeOutcome result =
...
opcode_deep_stack_family: {
  ...
  frame.next_pc = interpreter_cursor_pc(&frame, ip);
  struct tuple_uint_32_uint_64_StackPointer_OpcodeOutcome result =
      execute_deep_stack_encoded(frame.code, opcode, frame.next_pc, gas, sp);
  frame.next_pc = result.tup0;
  gas = result.tup1;
  sp = result.tup2;
  interpreter_load_cursor(&frame, &ip, &code_end);
...
opcode_return: {
  struct tuple_uint_64_StackPointer_Bytes_FrameStatus result = execute_return(gas, sp, memory);
  gas = result.tup0;
  sp = result.tup1;
  memory = result.tup2;
  status = result.tup3;
...
opcode_revert: {
  struct tuple_uint_64_uint_64_uint_32_StackPointer_Bytes_FrameStatus result = execute_revert(
      frame.state_gas_reservoir, gas, state_gas_remaining, state_gas_spilled, sp, memory);
  gas = result.tup0;
  state_gas_remaining = result.tup1;
  state_gas_spilled = result.tup2;
  sp = result.tup3;
...
opcode_selfdestruct: {
  struct tuple_uint_64_uint_64_uint_32_int_128_StackPointer_FrameStatus result =
      execute_selfdestruct(frame.address, fork, frame.is_static, gas, state_gas_remaining,
                           state_gas_spilled, refund, sp);
  gas = result.tup0;
  state_gas_remaining = result.tup1;
  state_gas_spilled = result.tup2;
