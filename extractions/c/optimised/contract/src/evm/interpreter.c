...
opcode_jump: {
  ...
  struct OpcodeOutcome outcome = execute_jump(frame.code, &frame.next_pc, &gas, &sp);
  interpreter_load_cursor(&frame, &ip, &code_end);
  FINISH_OPCODE(outcome);
}
opcode_jumpi: {
  frame.next_pc = interpreter_cursor_pc(&frame, ip);
  struct OpcodeOutcome outcome = execute_jumpi(frame.code, &frame.next_pc, &gas, &sp);
  interpreter_load_cursor(&frame, &ip, &code_end);
  FINISH_OPCODE(outcome);
}
opcode_pc: {
...
opcode_selfdestruct: {
  status = execute_selfdestruct(frame.address, fork, frame.is_static, &gas, &state_gas_remaining,
                                &state_gas_spilled, &refund, &sp);
  goto opcode_done;
}
