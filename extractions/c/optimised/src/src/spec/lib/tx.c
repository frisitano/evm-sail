...
uint8_t tx_signature_parity(uint64_t chain_id, enum TxSignatureScheme scheme, u256 v)
{
  ...
  case LegacySignature:
  {
    ...
    bool tmp_3_3524 = (bool)(u256_eq_u64(v, UINT8_C(27)) || (u256_eq_u64(v, UINT8_C(28)) || (eip155_v && u256_eq_u64(signature_chain_id, expected_chain_id))));
    if (tmp_3_3524) {
      u256 parity_bit = word_and(v, WORD_ONE);
      bool eq_int_result_2_2215 = eq_u256(parity_bit, WORD_ONE);
      if (eq_int_result_2_2215) {
        return UINT8_C(0);
      }
      return UINT8_C(1);
    }
  ...
  case TypedSignature:
  {
    bool eq_int_result_2_2216 = eq_u256(v, WORD_ZERO);
    if (eq_int_result_2_2216) {
      return UINT8_C(0);
    }
    bool eq_int_result_2_2217 = eq_u256(v, WORD_ONE);
    if (eq_int_result_2_2217) {
      return UINT8_C(1);
    }
    fatal_error(InvalidSignature);
  }
