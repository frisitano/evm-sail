...
Bytes encode_state_account(struct AccountInfo info, bytes32 storage_root)
{
  ...
  uint8_t balance_length = rlp_uint_word_size(info.balance);
  uint8_t content_len = ((uint8_t)((uint32_t)UINT8_C(33) + ((uint32_t)UINT8_C(33) + ((uint32_t)balance_length + (uint32_t)nonce_length))));
  ...
  u256 storage_root_word = hash_to_word(storage_root);
  rlp_write_word(storage_root_word);
  u256 code_hash_word = hash_to_word(info.code_hash);
