...
struct TrieUpdate account_update(struct AcctTrieEntry trie_entry, bytes32 storage_root)
{
  ...
  if (!current.present || account_empty) {
    struct TrieChange TrieDelete_result_2_527 = TrieDelete(UNIT);
    return ((struct TrieUpdate){.change = TrieDelete_result_2_527, .key = key});
  }
  Bytes encoded_account = encode_state_account(current.info, storage_root);
  struct TrieChange TriePut_result_2_528 = TriePut(encoded_account);
  return ((struct TrieUpdate){.change = TriePut_result_2_528, .key = key});
}
...
struct TrieUpdate storage_update(struct StorageTrieEntry trie_entry)
{
  ...
  bool value_is_zero = word_is_zero(entry.value.curr);
...
struct TrieUpdateFetch next_storage_trie_update(bytes20 addr)
{
  ...
  while (searching) {
    ...
    case Kind_StorageBlockIterRow:
    {
      ...
      if (changed) {
        struct TrieUpdate storage_update_result_2_430 = storage_update(trie_entry);
        result = ((struct TrieUpdateFetch){.available = true, .update = storage_update_result_2_430});
        searching = false;
      }
      break;
    }
