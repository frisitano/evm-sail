...
void write_prefix(bytes32 root, bool success)
{
  ...
  uint64_t success_byte = success ? UINT64_C(0x01) : UINT64_C(0x00);
...
{
  uint32_t output_length = scratch_length_add(((uint8_t)((uint32_t)RESULT_METADATA_LENGTH + (uint32_t)WORD_BYTE_LENGTH)), chain_config.len);
...
{
  commit_validation_result((htr_new_payload_request_(input_ref)), success, input_ref.chain_config);
}
