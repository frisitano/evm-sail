...
static void acquire_private_input(void)
{
  if (!private_input_ready) {
    ...
    if (!evmsail_capacity_prepare_input(private_input, private_input_size, &canonical_size))
      abort();
...
bool evmsail_stateless_input_contains(uint64_t pointer, uint64_t len)
{
  ...
  return private_input && address >= base && address - base <= private_input_size &&
...
const uint8_t *evmsail_code_ptr(uint64_t off, uint64_t len)
{
  ...
  if (len == 0)
    return &empty_region;
  if (!code_db_resolve_code(off, len, &bytes, &resolved_len) || resolved_len != len) {
    return NULL;
  }
  return bytes;
}
...
static uint64_t region_byte_at(span_resolver resolve, uint64_t off, uint64_t len, uint64_t index)
{
  if (index >= len)
    return 0;
...
static uint64_t region_count_nonzero(span_resolver resolve, uint64_t off, uint64_t len)
{
...
static bool region_strided_zero(span_resolver resolve, uint64_t off, uint64_t len, uint64_t start,
...
static u256 region_load_word(span_resolver resolve, uint64_t off, uint64_t len, uint64_t index)
{
  uint8_t bytes[32] = {0};
  if (index < len) {
    ...
    if (source)
      memcpy(bytes, source, (size_t)count);
  }
  return be_bytes_to_sail_word(bytes);
}

static u256 region_load_n_word(span_resolver resolve, uint64_t off, uint64_t len, uint64_t index,
                               uint64_t requested)
{
  uint8_t bytes[32] = {0};
  ...
  if (index < len) {
    ...
    const uint8_t *source = resolve(off + index, count);
    if (source)
      memcpy(bytes + sizeof(bytes) - width, source, (size_t)count);
  }
  return be_bytes_to_sail_word(bytes);
}

static unit region_copy_to_memory(span_resolver resolve, uint64_t off, uint64_t len, uint64_t dst,
...
#define DEFINE_SLICE_BYTE(prefix, type, resolver)                                                  \
  ...
  {                                                                                                \
    return region_byte_at(resolver, SLICE_OFF(slice), SLICE_LEN(slice), index);                    \
  }

#define DEFINE_SLICE_LOAD(prefix, type, resolver)                                                  \
  u256 prefix##_load_word(struct type slice, uint64_t index)                                       \
  {                                                                                                \
    return region_load_word(resolver, SLICE_OFF(slice), SLICE_LEN(slice), index);                  \
  }

#define DEFINE_SLICE_LOAD_N(prefix, type, resolver)                                                \
  u256 prefix##_load_n_word(struct type slice, uint64_t index, uint64_t len)                       \
  {                                                                                                \
    return region_load_n_word(resolver, SLICE_OFF(slice), SLICE_LEN(slice), index, len);           \
  }

#define DEFINE_SLICE_COPY(prefix, type, resolver)                                                  \
  unit prefix##_copy_to_memory(struct type slice, uint64_t dst, uint64_t index, uint64_t len)      \
  {                                                                                                \
    return region_copy_to_memory(resolver, SLICE_OFF(slice), SLICE_LEN(slice), dst, index, len);   \
  }

DEFINE_SLICE_BYTE(stateless_input, zStatelessInputSliceFields, evmsail_stateless_input_ptr)
...
DEFINE_SLICE_LOAD(stateless_input, zStatelessInputSliceFields, evmsail_stateless_input_ptr)
...
DEFINE_SLICE_LOAD_N(stateless_input, zStatelessInputSliceFields, evmsail_stateless_input_ptr)
...
DEFINE_SLICE_COPY(stateless_input, zStatelessInputSliceFields, evmsail_stateless_input_ptr)
...
uint64_t stateless_input_count_nonzero(struct zStatelessInputSliceFields slice)
{
  return region_count_nonzero(evmsail_stateless_input_ptr, SLICE_OFF(slice), SLICE_LEN(slice));
}

bool stateless_input_strided_zero(struct zStatelessInputSliceFields slice, uint64_t start,
                                  uint64_t stride, uint64_t width, uint64_t count)
{
  return region_strided_zero(evmsail_stateless_input_ptr, SLICE_OFF(slice), SLICE_LEN(slice), start,
                             stride, width, count);
}

bool memory_slice_strided_zero(struct zEvmMemorySliceFields slice, uint64_t start, uint64_t stride,
                               uint64_t width, uint64_t count)
{
  return region_strided_zero(evmsail_memory_ptr, SLICE_OFF(slice), SLICE_LEN(slice), start, stride,
                             width, count);
}

static bool regions_equal(span_resolver left_resolver, uint64_t left_off, uint64_t left_len,
                          span_resolver right_resolver, uint64_t right_off, uint64_t right_len)
{
                                ...
                                struct zStatelessInputSliceFields right)
{
  return regions_equal(evmsail_scratch_ptr, SLICE_OFF(left), SLICE_LEN(left),
                       evmsail_stateless_input_ptr, SLICE_OFF(right), SLICE_LEN(right));
}
...
{
  return regions_equal(evmsail_stateless_input_ptr, SLICE_OFF(left), SLICE_LEN(left),
                       evmsail_code_ptr, SLICE_OFF(right), SLICE_LEN(right));
}

bool evmsail_logs_bloom_matches_ref(fixed_bytes_256 computed,
                                    struct zStatelessInputSliceFields reference)
{
  const uint64_t reference_len = SLICE_LEN(reference);
  const uint8_t *reference_bytes = evmsail_stateless_input_ptr(SLICE_OFF(reference), reference_len);
...
static struct zScratchSliceFields scratch_result_value(uint64_t end)
{
...
static struct zScratchSliceFields scratch_store_region(uint64_t dst, span_resolver resolve,
...
#define DEFINE_SCRATCH_STORE(name, type, resolver)                                                 \
  struct zScratchSliceFields name(uint64_t off, struct type slice)                                 \
  {                                                                                                \
    return scratch_store_region(off, resolver, SLICE_OFF(slice), SLICE_LEN(slice));                \
  }

DEFINE_SCRATCH_STORE(scratch_store_stateless_input, zStatelessInputSliceFields,
                     evmsail_stateless_input_ptr)
DEFINE_SCRATCH_STORE(scratch_store_scratch, zScratchSliceFields, evmsail_scratch_ptr)
DEFINE_SCRATCH_STORE(scratch_store_log_data, zLogDataSliceFields, evmsail_log_data_ptr)
DEFINE_SCRATCH_STORE(scratch_store_output, zOutputSliceFields, evmsail_output_ptr)

#undef DEFINE_SCRATCH_STORE

struct zScratchSliceFields scratch_store_address(uint64_t off, fixed_bytes_20 data)
{
  const uint64_t dst = off;
...
struct zScratchSliceFields scratch_store_b256(uint64_t off, fixed_bytes_32 data, uint64_t len)
{
  const uint64_t dst = off;
  if (len > 32)
    abort();
...
struct zScratchSliceFields scratch_store_fixed_bytes_256(uint64_t off, fixed_bytes_256 data)
{
  const uint64_t dst = off;
...
struct zScratchSliceFields scratch_store_receipt_logs_bloom(uint64_t off, uint64_t start,
                                                            uint64_t count)
{
  const uint64_t dst = off;
  uint8_t *out = scratch_prepare(dst, 256);
...
struct zScratchSliceFields scratch_store_word(uint64_t off, const u256 data, uint64_t len)
{
  const uint64_t dst = off;
  if (len > 32)
    abort();
...
{
  return output_buffer_store_bytes(evmsail_memory_ptr(SLICE_OFF(slice), SLICE_LEN(slice)),
                                   SLICE_LEN(slice));
}

bool output_buffer_store_input(struct zStatelessInputSliceFields slice)
{
  return output_buffer_store_bytes(evmsail_stateless_input_ptr(SLICE_OFF(slice), SLICE_LEN(slice)),
                                   SLICE_LEN(slice));
}

fixed_bytes_32 code_db_store_indexed(struct zCodeFields code)
{
  return code_db_store_indexed_bytes(evmsail_code_ptr(code.zbytes, code.zlen), code.zlen,
                                     code.zjumpdests);
}

static bool calldata_span(struct zCalldataSlice input, const uint8_t **bytes, uint64_t *len)
{
  switch (input.kind) {
  case Kind_zInputCalldata:
    *len = SLICE_LEN(input.variants.zInputCalldata);
    *bytes = evmsail_stateless_input_ptr(SLICE_OFF(input.variants.zInputCalldata), *len);
    return *bytes != NULL;
  case Kind_zMemoryCalldata:
    *len = SLICE_LEN(input.variants.zMemoryCalldata);
    *bytes = evmsail_memory_ptr(SLICE_OFF(input.variants.zMemoryCalldata), *len);
    return *bytes != NULL;
  }
  return false;
}
...
bool accelerator_modexp(struct zCalldataSlice input, uint64_t base_len, uint64_t exponent_len,
...
bool accelerator_blake2f(struct zCalldataSlice input, uint64_t rounds, uint64_t final_block)
{
