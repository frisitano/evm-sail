...
static inline void evmsail_address_to_be_bytes(
    uint8_t out[20], fixed_bytes_20 value) {
  memcpy(out, value.bytes, sizeof(value.bytes));
}

static inline void evmsail_hash_to_be_bytes(
    uint8_t out[32], fixed_bytes_32 value) {
  memcpy(out, value.bytes, sizeof(value.bytes));
}

static inline fixed_bytes_20 evmsail_address_from_be_bytes(
    const uint8_t in[20]) {
  fixed_bytes_20 result;
  memcpy(result.bytes, in, sizeof(result.bytes));
  return result;
}

static inline fixed_bytes_32 evmsail_hash_from_be_bytes(
    const uint8_t in[32]) {
  fixed_bytes_32 result;
  memcpy(result.bytes, in, sizeof(result.bytes));
  return result;
}

static inline void evmsail_address_set_be_bytes(
    fixed_bytes_20 *out, const uint8_t in[20]) {
  *out = evmsail_address_from_be_bytes(in);
}

static inline void evmsail_hash_set_be_bytes(
    fixed_bytes_32 *out, const uint8_t in[32]) {
  *out = evmsail_hash_from_be_bytes(in);
}

static inline void evmsail_address_assign(
    fixed_bytes_20 *out, fixed_bytes_20 value) {
  *out = value;
}

static inline void evmsail_hash_assign(
    fixed_bytes_32 *out, fixed_bytes_32 value) {
  *out = value;
}
...
static inline void sail_hash_to_be_words4(uint64_t out[4], fixed_bytes_32 value) {
  uint8_t bytes[32];
...
static inline void sail_hash_to_le_words4(uint64_t out[4], fixed_bytes_32 value) {
  uint64_t words[4];
...
static inline fixed_bytes_32 be_words4_to_sail_hash(const uint64_t words[4]) {
  uint8_t bytes[32];
  fixed_bytes_32 result;
...
static inline fixed_bytes_32 le_words4_to_sail_hash(const uint64_t words[4]) {
  const uint64_t big_endian[4] = {words[3], words[2], words[1], words[0]};
  return be_words4_to_sail_hash(big_endian);
}

static inline fixed_bytes_20 be_bytes_to_sail_address(const uint8_t in[20]) {
  fixed_bytes_20 result;
