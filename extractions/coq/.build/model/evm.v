...
Definition account_changed (c : Account) (o : Account) : bool :=
   orb ((account_info_changed (c.(Account_info)) (o.(Account_info))))
     ((orb ((neq_bool (c.(Account_exists)) (o.(Account_exists))))
...
Definition store_account_info (a : mword 160) (acc : Account) (info : AccountInfo) : M (unit) :=
   ...
   (if orb
         ...
         ((orb ((neq_bool (next.(Account_exists)) (acc.(Account_exists))))
...
Definition k_account_exists (a : mword 160) : M (bool) :=
   (k_aload (a)) >>= fun (w__0 : Account) => returnM (w__0.(Account_exists)).
...
Definition k_get_codehash (a : mword 160) : M (mword 256) :=
   (k_aload (a)) >>= fun acc =>
   returnM ((if negb (acc.(Account_exists)) then WORD_ZERO
        ...
        | Some e =>
           ...
           let curr : Account :=
             {| Account_info := curr.(Account_info);
                Account_exists := curr.(Account_exists);
     ...
     ((orb
         ...
         ((orb
             ...
             ((orb
                 ...
                 ((orb
                     ((negb
                         ((Bool.eqb (value.(AcctValue_curr).(Account_exists))
                             (value.(AcctValue_orig).(Account_exists))))))
                     ((negb
...
Definition account_update (entry : AcctEntry) (storage : list TrieUpdate) : M (TrieUpdate) :=
   ...
   (if orb ((negb (current.(Account_exists)))) ((account_info_empty (current.(Account_info)))) then
