...
LogDataSliceLength: TypeAlias = LogDataSliceFields
    ...
    match s:
        case InputCalldata(bytes):
            if (int((int(off) + int(sail_len))) <= int(bytes.len)):
                subslice = stateless_input_sub_slice(bytes, off, sail_len)
        ...
        case MemoryCalldata(bytes):
            if (int((int(off) + int(sail_len))) <= int(bytes.len)):
                subslice = memory_sub_slice(bytes, off, sail_len)
                return MemoryCalldata(subslice)
            else:
                if not (False):
                    raise SailError("calldata sub-slice bounds")
                raise SailExit(None)
