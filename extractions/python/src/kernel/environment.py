...
from src.host.region_access import stateless_input_slice_load_n
from src.primitives.chain_config import (
    ExecutionProfile,
    DEFAULT_EXECUTION_PROFILE,
)
from src.primitives.tx import EMPTY_BLOB_HASHES
...
from src.lib.rlp.codecs.address import create_address
from src.lib.address import create2_address
...
def k_env(f: EnvField) -> word:
    ...
    match f:
        case EnvField.F_Number:
            number = word_of_block_number(k_header.number)
            return word(u256(number))
        case EnvField.F_Timestamp:
            timestamp = word_of_block_timestamp(k_header.timestamp)
            return word(u256(timestamp))
        ...
        case EnvField.F_ChainId:
            chain_id = word_of_chain_identifier(k_chain_id)
            return word(u256(chain_id))
...
def k_blockhash(number_word: word) -> hash:
    ...
    current_number = word_of_block_number(current)
    current_word = u256(current_number)
