...
def decode_state_account (value : ByteSlice) : SailM AccountInfo := do
...
def node_db_lookup (h : (BitVec 256)) : SailM ByteSlice := do
...
def decode_trie_node (node : ByteSlice) : SailM TrieNode := do
...
def inline_node_from_slice (bytes : ByteSlice) : SailM InlineNode := do
...
def inline_node_slice (node : InlineNode) : SailM ByteSlice := do
  let start ← do (scratch_begin ())
  (scratch_push_bytes (inline_node_to_list node) node.len)
  (scratch_finish start)

def resolve_ref (r : NodeRef) : SailM ByteSlice := do
  match r with
...
def trie_walk (node : ByteSlice) (key : TriePath) (pos : Nat) (fuel : Nat) : SailM ByteSlice := do
...
termination_by let (node, key, pos, fuel) := (node, key, pos, fuel); (fuel).toNat

def trie_lookup (root : (BitVec 256)) (key : TriePath) : SailM ByteSlice := do
