...
def calldata_install (data : ByteSlice) : SailM Unit := do
...
def memory_byte_slice (off : byte_quantity) (len : byte_quantity) : SailM ByteSlice := do
...
def memory_frame_enter (_ : Unit) : SailM ByteSlice := do
...
def memory_frame_leave (parent : ByteSlice) : SailM Unit := do
  (mem_frame_leave ())
