...
def execute_block_transactions (transactions : SszListRef) (public_keys : ByteSlice) (block_gas_limit : gas) : SailM BlockExecutionResult := do
  ...
  let (all_ok, blob_gas_acc, blob_gas_overflow, block_gas_overflow, cursor, gas_acc, keys, receipts, tx0_to) ← (( do
    ...
    (Bool × Nat × Bool × Bool × SszListCursor × gas × ByteSlice × ReceiptAccumulator × (BitVec 160))
    )
...
/-- Type quantifiers: k_ex173105_ : Bool -/
def run_checked_block_end_system_calls (all_ok : Bool) (deposits : ByteSlice) : SailM (Bool × ExecutionRequests) := do
  if ((! all_ok) : Bool)
...
def execute_block_body (body : BlockBody) (public_keys : ByteSlice) (block_gas_limit : gas) : SailM (Bool × BlockExecutionResult) := do
  (bal_reset ())
  (bal_set_index 0)
