...
def ssz_u32_at (input : ByteSlice) (offset : byte_quantity) : SailM Nat := do
...
def ssz_u32 (input : ByteSlice) (offset : byte_quantity) : SailM Nat := do
  (ssz_u32_at input offset)

def ssz_limb (input : ByteSlice) (offset : byte_quantity) : SailM (BitVec 64) := do
  let b0 ← do (pure (Sail.BitVec.zeroExtend (← (slice_byte input offset)) 64))
  let b1 ← do
    (pure (Sail.BitVec.zeroExtend (← (slice_byte input (← (ssz_field_offset offset 1)))) 64))
  let b2 ← do
...
def ssz_uint (input : ByteSlice) (offset : byte_quantity) : SailM Nat := do
  (pure (BitVec.toNatInt (← (ssz_limb input offset))))

def ssz_addr (input : ByteSlice) (offset : byte_quantity) : SailM (BitVec 160) := do
  (pure (Sail.BitVec.extractLsb (← (slice_load_n input offset ADDRESS_BYTE_LENGTH)) 159 0))

def ssz_bytes32 (input : ByteSlice) (offset : byte_quantity) : SailM (BitVec 256) := do
  (slice_load input offset)

def ssz_logs_bloom (input : ByteSlice) (offset : byte_quantity) : SailM (Vector (BitVec 64) 32) := do
...
def ssz_u256 (input : ByteSlice) (offset : byte_quantity) : SailM (BitVec 256) := do
