...
def path_prefix_of (prefix : TriePath) (path : TriePath) : Bool :=
  if (((path_len path) <b (path_len prefix)) : Bool)
  then false
...
def hex_prefix_pairs (path : TriePath) (index : Nat) : (List (BitVec 8)) :=
  if (((path_len path) ≤b index) : Bool)
  then []
  else
    (let next := (index +i 1)
    (((path_nibble path index) ++ (path_nibble path next)) :: (hex_prefix_pairs path (next +i 1))))
termination_by let (path, index) := (path, index); ((65 -i index)).toNat

/-- Type quantifiers: k_ex163093_ : Bool -/
