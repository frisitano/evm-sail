...
def child_ref (encoded : ByteSlice) : SailM NodeRef := do
...
def leaf_child_ref (key : TriePath) (value : ByteSlice) : SailM NodeRef := do
...
def node_to_ref (node : ByteSlice) : SailM NodeRef := do
...
def merge_ext_node (prefix : TriePath) (childnode : ByteSlice) : SailM NodeRef := do
  if (((path_len prefix) == 0) : Bool)
    ...
    (do
      ...
      then (pure (EmptyRef ()))
        ...
        (do
          ...
          | .LeafNode leaf =>
            (leaf_child_ref (← (path_concat prefix leaf.path)) (← (rlp_ref_content leaf.value)))
          | .ExtensionNode ext =>
            (extension_child_ref (← (path_concat prefix ext.path)) (← (field_to_ref ext.child)))
          | _ => (extension_child_ref prefix (← (node_to_ref childnode)))))

def merge_ext_ref (prefix : TriePath) (childref : NodeRef) : SailM NodeRef := do
  if (((path_len prefix) == 0) : Bool)
  then (pure childref)
  else
    (do
