...
def precompile_success (output : ByteSlice) : PrecompileResult :=
  { success := true
    output := output }
...
/-- Type quantifiers: k_ex162014_ : Bool -/
...
def copied_result (data : ByteSlice) : SailM PrecompileResult := do
...
/-- Type quantifiers: k_ex162017_ : Bool -/
...
def run_ecrecover (input : ByteSlice) : SailM PrecompileResult := do
...
def run_sha256 (input : ByteSlice) : SailM PrecompileResult := do
  (pure (precompile_success (← (output_buffer_word (← (sha256_segments [(BytesSlice input)]))))))

def run_ripemd160 (input : ByteSlice) : SailM PrecompileResult := do
  (pure (accelerator_result (← (accelerator_ripemd160 input)) PRECOMPILE_WORD_LENGTH))

def run_modexp (input : ByteSlice) : SailM PrecompileResult := do
...
def run_blake2f (input : ByteSlice) : SailM PrecompileResult := do
...
def kzg_versioned_hash_matches (input : ByteSlice) : SailM Bool := do
...
def run_kzg_point_evaluation (input : ByteSlice) : SailM PrecompileResult := do
...
def bls_g1_padding (input : ByteSlice) (base : byte_quantity) (stride : byte_quantity) (count : byte_quantity) : SailM Bool := do
...
def bls_g2_padding (input : ByteSlice) (base : byte_quantity) (stride : byte_quantity) (count : byte_quantity) : SailM Bool := do
  (pure ((← (slice_strided_zero input base stride BLS_FIELD_PADDING_LENGTH count)) && ((← (slice_strided_zero
            input (← (byte_quantity_add base BLS_PADDED_FIELD_LENGTH)) stride
...
def run_bls_g1_add (input : ByteSlice) : SailM PrecompileResult := do
...
def run_bls_g1_msm (input : ByteSlice) : SailM PrecompileResult := do
...
def run_bls_g2_add (input : ByteSlice) : SailM PrecompileResult := do
...
def run_bls_g2_msm (input : ByteSlice) : SailM PrecompileResult := do
  let length := input.len
...
def run_bls_pairing (input : ByteSlice) : SailM PrecompileResult := do
  let length := input.len
...
def run_bls_map_fp_to_g1 (input : ByteSlice) : SailM PrecompileResult := do
...
def run_bls_map_fp2_to_g2 (input : ByteSlice) : SailM PrecompileResult := do
...
def run_p256_verify (input : ByteSlice) : SailM PrecompileResult := do
  let verified ← do
...
def run_precompile_slice (num : Nat) (input : ByteSlice) : SailM PrecompileResult := do
  match num with
