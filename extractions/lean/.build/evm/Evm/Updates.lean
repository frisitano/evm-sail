...
def next_update_under (updates : (List TrieUpdate)) (prefix : TriePath) : Bool :=
  match updates with
  | (update :: _) => (path_prefix_of prefix update.key)
  | [] => false

def item_leaf (path : TriePath) (value : ByteSlice) : TrieItem :=
  { path := path
...
termination_by let (builder, anchor, next_common, fuel) := (builder, anchor, next_common, fuel); (fuel).toNat
