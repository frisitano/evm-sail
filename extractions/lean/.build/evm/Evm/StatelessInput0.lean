...
def ssz_list_pop (cursor : SszListCursor) : SailM (ByteSlice × SszListCursor) := do
...
/-- Type quantifiers: index : Nat, 0 ≤ index ∧ index ≤ (2 ^ 64 - 1) -/
def ssz_list_at (items : SszListRef) (index : Nat) : SailM ByteSlice := do
...
/-- Type quantifiers: index : Nat, 0 ≤ index ∧ index ≤ (2 ^ 64 - 1) -/
def ssz_fixed_list_at (items : SszListRef) (index : Nat) (item_size : byte_quantity) : SailM ByteSlice := do
  if ((items.count ≤b index) : Bool)
  then sailThrow ((InvalidBlock InvalidConfig))
  else (pure ())
  let width := item_size
...
def ssz_fixed_list_pop (items : SszListRef) (item_size : byte_quantity) : SailM (ByteSlice × SszListRef) := do
  let width := item_size
...
def ssz_span (input : ByteSlice) (start : byte_quantity) (stop : byte_quantity) : SailM ByteSlice := do
  if (((byte_quantity_gt start stop) || (byte_quantity_gt stop input.len)) : Bool)
  then sailThrow ((InvalidBlock InvalidConfig))
  else (pure ())
  (sub_slice input start (← (byte_quantity_sub stop start)))

def ssz_variable_list_ref (input : ByteSlice) (start : byte_quantity) (stop : byte_quantity) : SailM SszListRef := do
...
def ssz_fixed_list_ref (input : ByteSlice) (start : byte_quantity) (stop : byte_quantity) (item_size : byte_quantity) : SailM SszListRef := do
  let bytes ← do (ssz_span input start stop)
...
def decode_stateless_input_ref (input : ByteSlice) : SailM StatelessInputRef := do
...
def sha256_request_digest (request_type : (BitVec 8)) (s : ByteSlice) : SailM (BitVec 256) := do
...
termination_by let cursor := cursor; ((cursor.items.count -i cursor.index)).toNat
...
def decode_parent_header_fields (cursor : RlpCursor) (field_index : Nat) (fields : ParentHeaderFields) : SailM ParentHeaderFields := do
...
termination_by let (cursor, field_index, fields) := (cursor, field_index, fields); (let .ByteQuantity stop :=
  cursor.stop
...
termination_by let state := state; ((state.cursor.items.count -i state.cursor.index)).toNat
...
def decode_withdrawal (withdrawal : ByteSlice) : SailM Withdrawal := do
...
def decode_chain_config (cc : ByteSlice) (number : Nat) (timestamp : Nat) : SailM ChainConfig := do
  let cc_end := cc.len
...
def decode_transaction (transaction : ByteSlice) (public_key : ByteSlice) : SailM Transaction := do
