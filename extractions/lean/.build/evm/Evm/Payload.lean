...
def withdrawal_rlp (withdrawal : ByteSlice) : SailM ByteSlice := do
  let index ← do (ssz_uint withdrawal WD_INDEX)
  let validator_index ← do (ssz_uint withdrawal WD_VALIDATOR_INDEX)
  let address ← do (sub_slice withdrawal WD_ADDRESS ADDRESS_BYTE_LENGTH)
  let amount ← do (ssz_uint withdrawal WD_AMOUNT)
  ...
  let content_len ← (byte_quantity_add content_len (← (rlp_protocol_quantity_size amount)))
...
def block_header_hash (header : BlockHeader) (transactions_root : (BitVec 256)) (withdrawals_root : (BitVec 256)) (requests_hash : (BitVec 256)) (block_access_list_hash : (BitVec 256)) : SailM (BitVec 256) := do
  let bloom := (logs_bloom_bytes header.logs_bloom)
  let nonce : (List byte) := [0x00#8, 0x00#8, 0x00#8, 0x00#8, 0x00#8, 0x00#8, 0x00#8, 0x00#8]
  ...
  let content_len ←
    (byte_quantity_add content_len (← (rlp_protocol_quantity_size header.number)))
  ...
  let content_len ←
    (byte_quantity_add content_len (← (rlp_protocol_quantity_size header.timestamp)))
  ...
  let content_len ← (( do
    ...
    then
      (do
        let content_len ←
          (byte_quantity_add content_len (← (rlp_protocol_quantity_size header.blob_gas_used)))
        let content_len ←
          (byte_quantity_add content_len (← (rlp_protocol_quantity_size header.excess_blob_gas)))
        (byte_quantity_add content_len (rlp_word_size ())))
    else (pure content_len) ) : SailM byte_quantity )
  let content_len ← (( do
  ...
  let content_len ← (( do
    ...
    then
      (do
        let content_len ← (byte_quantity_add content_len (rlp_word_size ()))
        (byte_quantity_add content_len (← (rlp_protocol_quantity_size header.slot_number))))
    else (pure content_len) ) : SailM byte_quantity )
  let encoded_len ← do (rlp_list_size content_len)
  ...
  (rlp_write_protocol_quantity header.number)
  (rlp_write_gas header.gas_limit)
  (rlp_write_gas header.gas_used)
  (rlp_write_protocol_quantity header.timestamp)
  ...
  then
    (do
      (rlp_write_protocol_quantity header.blob_gas_used)
      (rlp_write_protocol_quantity header.excess_blob_gas)
  ...
  then
    (do
      ...
      (rlp_write_protocol_quantity header.slot_number))
  else (pure ())
...
def transaction_trie_root (txs : SszListRef) : SailM (BitVec 256) := do
  let builder := (trie_builder_empty ())
  let cursor := (rlp_index_cursor txs.count)
  let (builder, cursor) ← (( do
    let mut loop_vars := (builder, cursor)
    while (λ (builder, cursor) => (! (rlp_index_cursor_empty cursor))) loop_vars do
      let (builder, cursor) := loop_vars
      loop_vars ← do
        let (item, next) ← do (rlp_index_cursor_pop cursor)
        let cursor : RlpIndexCursor := next
        let tx ← do (ssz_list_at txs item.index)
...
def withdrawals_trie_root (wds : SszListRef) : SailM (BitVec 256) := do
  let builder := (trie_builder_empty ())
  let cursor := (rlp_index_cursor wds.count)
  let (builder, cursor) ← (( do
    let mut loop_vars := (builder, cursor)
    while (λ (builder, cursor) => (! (rlp_index_cursor_empty cursor))) loop_vars do
      let (builder, cursor) := loop_vars
      loop_vars ← do
        ...
        let withdrawal ← do (ssz_fixed_list_at wds item.index WD_SIZE)
...
def execution_requests_hash (input_ref : StatelessInputRef) : SailM (BitVec 256) := do
  ...
  let d0 ← (( do
    ...
    else (pure (BitVec.zero 256)) ) : SailM hash )
  let d1 ← (( do
...
def validate_execution_payload (input : StatelessInput) (input_ref : StatelessInputRef) (witness : WitnessContext) : SailM Unit := do
  ...
  let body := block.body
  if ((! ((9 ≤b input.chain_config.fork_index) && (((input.chain_config.fork_index ≤b 20) && (input.chain_config.activation_active && (chain_config_blob_schedule_valid
  ...
  if (((fork_gteq (← readReg k_fork) Cancun) && (← do
         (pure (header.excess_blob_gas != (← (expected_payload_excess_blob_gas witness)))))) : Bool)
