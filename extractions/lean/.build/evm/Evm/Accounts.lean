...
def account_changed (c : Account) (o : Account) : Bool :=
  ((account_info_changed c.info o.info) || ((neq_bool c.exists o.exists) || (neq_bool
...
def account_set_info (acc : Account) (info : AccountInfo) : Account :=
  ...
  then
    { acc with info := { EMPTY_ACCOUNT_INFO with storage_root := acc.info.storage_root }, exists := false, storage_cleared := true }
  else { acc with info := info, exists := true }
...
def store_account_info (a : (BitVec 160)) (acc : Account) (info : AccountInfo) : SailM Unit := do
  ...
  if (((next.info.storage_root != acc.info.storage_root) || ((neq_bool next.exists acc.exists) || (neq_bool
