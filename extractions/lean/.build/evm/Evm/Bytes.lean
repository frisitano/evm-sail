...
def byte_slice (src : ByteSource) (off : byte_quantity) (len : byte_quantity) : ByteSlice :=
  { source := src
    off := off
    len := len }

def EMPTY_SLICE : ByteSlice :=
  { source := StatelessInputSource
    off := BYTE_ZERO
    len := BYTE_ZERO }

def stateless_input_byte_slice (off : byte_quantity) (len : byte_quantity) : ByteSlice :=
  (byte_slice StatelessInputSource off len)

def sub_slice (s : ByteSlice) (off : byte_quantity) (len : byte_quantity) : SailM ByteSlice := do
...
def code_byte_slice (off : byte_quantity) (len : byte_quantity) : ByteSlice :=
