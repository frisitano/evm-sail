...
def enter_system_call_frame (tgt : (BitVec 160)) (input : ByteSlice) : SailM StateCheckpoint := do
...
def system_call_checked (tgt : (BitVec 160)) : SailM (Option ByteSlice) := do
  if (((← (k_code_key tgt)) == KECCAK_EMPTY) : Bool)
  then (pure none)
  else
    (do
      ...
      let result ← (( do
        ...
        else (pure none) ) : SailM (Option ByteSlice) )
      (memory_frame_leave parent_memory)
      if ((! ok) : Bool)
      then (k_revert checkpoint)
      else (pure ())
...
def append_deposit_request (data : ByteSlice) : SailM Unit := do
...
def collect_execution_requests (deposits : ByteSlice) : SailM (Bool × ExecutionRequests) := do
