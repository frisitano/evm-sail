...
def bal_storage_change_run_end (account : Nat) (count : Nat) (slot : (BitVec 256)) (index : Nat) (cursor : Nat) : SailM Nat := do
...
termination_by let (account, count, slot, index, cursor) := (account, count, slot, index, cursor); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_storage_slot_changes_size (account : Nat) (count : Nat) (slot : (BitVec 256)) (cursor : Nat) (content_len : byte_quantity) : SailM BalContentCursor := do
  if ((cursor <b count) : Bool)
  then
    (do
    ...
    (pure { content_len := content_len
            cursor := cursor })
termination_by let (account, count, slot, cursor, content_len) := (account, count, slot, cursor, content_len); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_write_storage_slot_changes (account : Nat) (count : Nat) (slot : (BitVec 256)) (cursor : Nat) : SailM Nat := do
  if ((cursor <b count) : Bool)
  then
    (do
      if (((← (bal_storage_change_slot account cursor)) == slot) : Bool)
...
termination_by let (account, count, slot, cursor) := (account, count, slot, cursor); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_storage_change_groups_size (account : Nat) (count : Nat) (cursor : Nat) (result : BalContentCount) : SailM BalContentCount := do
  if ((cursor <b count) : Bool)
  then
    (do
...
termination_by let (account, count, cursor, result) := (account, count, cursor, result); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_write_storage_change_groups (account : Nat) (count : Nat) (cursor : Nat) : SailM Unit := do
  if ((cursor <b count) : Bool)
  then
    (do
      let slot ← do (bal_storage_change_slot account cursor)
...
termination_by let (account, count, cursor) := (account, count, cursor); ((count -i cursor)).toNat
...
/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_storage_read_run_end (account : Nat) (count : Nat) (slot : (BitVec 256)) (cursor : Nat) : SailM Nat := do
  if ((cursor <b count) : Bool)
  then
    (do
...
/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_storage_change_seek (account : Nat) (count : Nat) (slot : (BitVec 256)) (cursor : Nat) : SailM Nat := do
  if ((cursor <b count) : Bool)
  then
    (do
...
/-- Type quantifiers: change : Nat, read : Nat, change_count : Nat, read_count : Nat, account : Nat, 0
  ...
  ∧ change ≤ (2 ^ 64 - 1) -/
def bal_storage_read_groups_size (account : Nat) (read_count : Nat) (change_count : Nat) (read : Nat) (change : Nat) (result : BalContentCount) : SailM BalContentCount := do
...
termination_by let (account, read_count, change_count, read, change, result) := (account, read_count, change_count, read, change, result); ((read_count -i read)).toNat

/-- Type quantifiers: change : Nat, read : Nat, change_count : Nat, read_count : Nat, account : Nat, 0
  ≤ account ∧ account ≤ (2 ^ 64 - 1), 0 ≤ read_count ∧ read_count ≤ (2 ^ 64 - 1), 0 ≤
  change_count ∧ change_count ≤ (2 ^ 64 - 1), 0 ≤ read ∧ read ≤ (2 ^ 64 - 1), 0 ≤ change
  ∧ change ≤ (2 ^ 64 - 1) -/
def bal_write_storage_read_groups (account : Nat) (read_count : Nat) (change_count : Nat) (read : Nat) (change : Nat) : SailM Unit := do
  if ((read <b read_count) : Bool)
  then
    (do
      let slot ← do (bal_storage_read_slot account read)
...
termination_by let (account, read_count, change_count, read, change) := (account, read_count, change_count, read, change); ((read_count -i read)).toNat
...
/-- Type quantifiers: cursor : Nat, index : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ index ∧
  index ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧ cursor ≤ (2 ^ 64 - 1) -/
def bal_balance_run_end (account : Nat) (count : Nat) (index : Nat) (cursor : Nat) : SailM Nat := do
  if ((cursor <b count) : Bool)
  then
    (do
...
termination_by let (account, count, index, cursor) := (account, count, index, cursor); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_balance_groups_size (account : Nat) (count : Nat) (cursor : Nat) (content_len : byte_quantity) : SailM byte_quantity := do
  if ((cursor <b count) : Bool)
  then
    (do
...
termination_by let (account, count, cursor, content_len) := (account, count, cursor, content_len); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_write_balance_groups (account : Nat) (count : Nat) (cursor : Nat) : SailM Unit := do
  if ((cursor <b count) : Bool)
  then
    (do
      let index ← do (bal_balance_change_index account cursor)
...
/-- Type quantifiers: maximum : Nat, cursor : Nat, index : Nat, count : Nat, account : Nat, 0 ≤
  ...
  maximum ≤ (2 ^ 64 - 1) -/
def bal_nonce_run (account : Nat) (count : Nat) (index : Nat) (cursor : Nat) (maximum : Nat) : SailM BalNonceRun := do
  if ((cursor <b count) : Bool)
  then
    (do
    ...
    (pure { cursor := cursor
            maximum := maximum })
termination_by let (account, count, index, cursor, maximum) := (account, count, index, cursor, maximum); ((count -i cursor)).toNat

/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_nonce_groups_size (account : Nat) (count : Nat) (cursor : Nat) (content_len : byte_quantity) : SailM byte_quantity := do
  if ((cursor <b count) : Bool)
  then
    (do
...
/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_write_nonce_groups (account : Nat) (count : Nat) (cursor : Nat) : SailM Unit := do
  if ((cursor <b count) : Bool)
  then
    (do
      let index ← do (bal_nonce_change_index account cursor)
...
/-- Type quantifiers: cursor : Nat, index : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ index ∧
  index ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧ cursor ≤ (2 ^ 64 - 1) -/
def bal_code_run_end (account : Nat) (count : Nat) (index : Nat) (cursor : Nat) : SailM Nat := do
  if ((cursor <b count) : Bool)
  then
    (do
...
/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_code_groups_size (account : Nat) (count : Nat) (cursor : Nat) (content_len : byte_quantity) : SailM byte_quantity := do
  if ((cursor <b count) : Bool)
  then
    (do
...
/-- Type quantifiers: cursor : Nat, count : Nat, account : Nat, 0 ≤ account ∧
  account ≤ (2 ^ 64 - 1), 0 ≤ count ∧ count ≤ (2 ^ 64 - 1), 0 ≤ cursor ∧
  cursor ≤ (2 ^ 64 - 1) -/
def bal_write_code_groups (account : Nat) (count : Nat) (cursor : Nat) : SailM Unit := do
  if ((cursor <b count) : Bool)
  then
    (do
      let index ← do (bal_code_change_index account cursor)
...
def bal_accounts_size (account_count : Nat) (account : Nat) (result : BalContentCount) : SailM BalContentCount := do
...
termination_by let (account_count, account, result) := (account_count, account, result); ((account_count -i account)).toNat

/-- Type quantifiers: account : Nat, account_count : Nat, 0 ≤ account_count ∧
  account_count ≤ (2 ^ 64 - 1), 0 ≤ account ∧ account ≤ (2 ^ 64 - 1) -/
def bal_write_accounts (account_count : Nat) (account : Nat) : SailM Unit := do
  if ((account <b account_count) : Bool)
  then
    (do
...
termination_by let (account_count, account) := (account_count, account); ((account_count -i account)).toNat
