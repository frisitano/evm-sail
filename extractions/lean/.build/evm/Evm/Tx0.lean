...
def tx_signing_hash (t : TxType) (content_src : ByteSlice) (v : (BitVec 256)) : SailM (BitVec 256) := do
  ...
  then
    (do
      ...
      let signing_hash ← do
        (keccak256_segments
          [(bytes_list prefix prefix_len), (BytesSlice content_src), (BytesSlice suffix)])
      (scratch_rewind mark)
      (pure signing_hash))
  else
    (do
      ...
      then
        (keccak256_segments
          [(bytes_list [tb] BYTE_ONE), (bytes_list prefix prefix_len), (BytesSlice content_src)])
      else (keccak256_segments [(bytes_list prefix prefix_len), (BytesSlice content_src)]))
...
def tx_auth_ok (pubkey : ByteSlice) (h : (BitVec 256)) (r : (BitVec 256)) (s : (BitVec 256)) : SailM Bool := do
...
termination_by let cursor := cursor; (let .ByteQuantity stop := cursor.stop
...
def tx_input_span (payload : ByteSlice) (data : RlpFieldRef) : SailM ByteSlice := do
  (sub_slice payload data.content_off data.content_len)

def tx_sig_span (payload : ByteSlice) (first : RlpFieldRef) (signature : RlpFieldRef) : SailM ByteSlice := do
...
def rlp_decode_tx (tx : ByteSlice) (pubkey : ByteSlice) (fork : Fork) : SailM Transaction := do
  let sender ← do
