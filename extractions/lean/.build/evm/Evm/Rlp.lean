...
def minimal_protocol_quantity_bytes (n : Nat) : SailM ((List (BitVec 8)) × Nat) := do
  let remaining : Nat := n
  let out : (List byte) := []
  let len : Nat := 0
    ...
    for byte_index in [loop_byte_index_lower:loop_byte_index_upper:1]i do
      ...
      loop_vars ← do
        let (len, out, remaining) ← (( do
          ...
          then
            (do
              let out : (List byte) := ((get_slice_int 8 (Int.tmod remaining 256) 0) :: out)
              let remaining ← (exact_quotient remaining 256)
              let len : Nat := (byte_index +i 1)
...
def minimal_gas_bytes (app_0 : gas) : SailM ((List (BitVec 8)) × Nat) := do
  let .Gas n := app_0
    ...
    for byte_index in [loop_byte_index_lower:loop_byte_index_upper:1]i do
      ...
      loop_vars ← do
        ...
        let (len, out, remaining) ← (( do
          ...
          then
            (do
              let out : (List byte) := ((get_slice_int 8 (Int.tmod remaining_value 256) 0) :: out)
...
def minimal_word_bytes (w : (BitVec 256)) : ((List (BitVec 8)) × Nat) := Id.run do
    ...
    for byte_index in [loop_byte_index_lower:loop_byte_index_upper:1]i do
      ...
      loop_vars :=
        let (len, out, remaining) : (Nat × (List (BitVec 8)) × (BitVec 256)) :=
          ...
          then
            (let out : (List byte) := ((Sail.BitVec.extractLsb remaining 7 0) :: out)
            let remaining : word := (remaining >>> 8)
            let len : Nat := (byte_index +i 1)
            (len, out, remaining))
...
def rlp_byte_length_byte (value : byte_quantity) : SailM (BitVec 8) := do
...
def rlp_length_word (value : byte_quantity) : SailM (BitVec 256) := do
  (word_of_byte_quantity value)

def rlp_string_prefix (len : byte_quantity) (first : (BitVec 8)) : SailM ((List (BitVec 8)) × byte_quantity) := do
...
def rlp_list_prefix (len : byte_quantity) : SailM ((List (BitVec 8)) × byte_quantity) := do
  if ((byte_quantity_le len RLP_SHORT_LENGTH_LIMIT) : Bool)
...
def rlp_minimal_word_len (w : (BitVec 256)) : Nat := Id.run do
  let remaining : word := w
  let len : Nat := 0
...
/-- Type quantifiers: n : Nat, 0 ≤ n ∧ n ≤ (2 ^ 64 - 1) -/
def rlp_minimal_protocol_quantity_len (n : Nat) : SailM Nat := do
  let remaining : Nat := n
  let len : Nat := 0
  let (len, remaining) ← (( do
    let loop_byte_index_lower := 0
...
def rlp_length_prefix_len (len : byte_quantity) : SailM byte_quantity := do
  if ((byte_quantity_le len RLP_SHORT_LENGTH_LIMIT) : Bool)
  then (pure BYTE_ONE)
  else (pure (ByteQuantity (1 +i (rlp_minimal_word_len (← (rlp_length_word len))))))

def rlp_string_size (len : byte_quantity) (first : (BitVec 8)) : SailM byte_quantity := do
  if (((byte_quantity_equal len BYTE_ONE) && ((BitVec.access first 7) == 0#1)) : Bool)
  then (pure BYTE_ONE)
  else (byte_quantity_add (← (rlp_length_prefix_len len)) len)

def rlp_bytes_size (data : (List (BitVec 8))) (len : byte_quantity) : SailM byte_quantity := do
  let first : byte :=
    match data with
    | (byte :: _) => byte
    | [] => 0x00#8
...
def rlp_slice_size (data : ByteSlice) : SailM byte_quantity := do
...
def rlp_uint_word_size (w : (BitVec 256)) : SailM byte_quantity := do
...
/-- Type quantifiers: n : Nat, 0 ≤ n ∧ n ≤ (2 ^ 64 - 1) -/
def rlp_protocol_quantity_size (n : Nat) : SailM byte_quantity := do
...
def rlp_gas_size (value : gas) : SailM byte_quantity := do
  let .Gas n := value
...
def rlp_word_size (_ : Unit) : byte_quantity :=
  RLP_ENCODED_WORD_LENGTH

def rlp_addr_size (_ : Unit) : byte_quantity :=
  RLP_ENCODED_ADDRESS_LENGTH

def rlp_list_size (content_len : byte_quantity) : SailM byte_quantity := do
  (byte_quantity_add (← (rlp_length_prefix_len content_len)) content_len)

def rlp_write_raw_bytes (data : (List (BitVec 8))) (len : byte_quantity) : SailM Unit := do
  (scratch_push_bytes data len)

def rlp_write_raw_slice (data : ByteSlice) : SailM Unit := do
  (scratch_push_slice data)

def rlp_write_string_prefix (len : byte_quantity) (first : (BitVec 8)) : SailM Unit := do
  let (prefix, prefix_len) ← do (rlp_string_prefix len first)
  (scratch_push_bytes prefix prefix_len)

def rlp_write_list_prefix (content_len : byte_quantity) : SailM Unit := do
  let (prefix, prefix_len) ← do (rlp_list_prefix content_len)
  (scratch_push_bytes prefix prefix_len)

def rlp_write_bytes (data : (List (BitVec 8))) (len : byte_quantity) : SailM Unit := do
  let first : byte :=
    match data with
    | (byte :: _) => byte
    | [] => 0x00#8
...
def rlp_write_slice (data : ByteSlice) : SailM Unit := do
...
def rlp_write_uint_word (w : (BitVec 256)) : SailM Unit := do
  let (bytes, len) := (minimal_word_bytes w)
  (rlp_write_bytes bytes (ByteQuantity len))

/-- Type quantifiers: n : Nat, 0 ≤ n ∧ n ≤ (2 ^ 64 - 1) -/
def rlp_write_protocol_quantity (n : Nat) : SailM Unit := do
...
def rlp_write_word (w : (BitVec 256)) : SailM Unit := do
  (rlp_write_bytes (word_to_bytes32 w) WORD_BYTE_LENGTH)

def rlp_write_addr (a : (BitVec 160)) : SailM Unit := do
  (rlp_write_bytes (address_to_bytes a) ADDRESS_BYTE_LENGTH)

def rlp_finish (start : byte_quantity) (expected_len : byte_quantity) : SailM ByteSlice := do
  let encoded ← do (scratch_finish start)
  assert (byte_quantity_equal encoded.len expected_len) "RLP forward writer length"
  (pure encoded)

def rlp_checked_add_quantity (a : byte_quantity) (b : byte_quantity) : SailM byte_quantity := do
...
/-- Type quantifiers: b : Nat, 0 ≤ b -/
def rlp_checked_add_nat (a : byte_quantity) (b : Nat) : SailM byte_quantity := do
  (rlp_checked_add_quantity a (ByteQuantity b))

/-- Type quantifiers: count : Nat, 0 ≤ count ∧ count ≤ 8 -/
def rlp_ref_be_length (source : ByteSlice) (start : byte_quantity) (count : Nat) : SailM byte_quantity := do
  let value : limb := LIMB_ZERO
  let current : source_pointer := start
...
def rlp_bytes_equal_at (expected : (List (BitVec 8))) (source : ByteSlice) (start : byte_quantity) : SailM Bool := do
  let rest : (List byte) := expected
  let current : source_pointer := start
  let equal : Bool := true
    ...
    for byte_index in [loop_byte_index_lower:loop_byte_index_upper:1]i do
      ...
      loop_vars ← do
        let (current, equal, rest) ← (( do
          ...
          | (byte :: tail) =>
            (do
              let equal ← (pure (equal && ((← (slice_byte source current)) == byte)))
              let current ← (rlp_checked_add_quantity current BYTE_ONE)
...
def rlp_ref_hdr (b : ByteSlice) (pos : byte_quantity) (stop : byte_quantity) : SailM (Bool × byte_quantity × byte_quantity) := do
...
/-- Type quantifiers: k_ex160514_ : Bool -/
def rlp_field_ref (source : ByteSlice) (is_list : Bool) (full_off : byte_quantity) (full_len : byte_quantity) (content_off : byte_quantity) (content_len : byte_quantity) : RlpFieldRef :=
  { source := source
    is_list := is_list
    full_off := full_off
    full_len := full_len
...
def rlp_invalid_cursor (source : ByteSlice) : RlpCursor :=
  { source := source
    current := BYTE_ZERO
    stop := BYTE_ZERO
    valid := false }

def rlp_node_cursor (node : ByteSlice) : SailM RlpCursor := do
  let n := node.len
...
def rlp_cursor_pop (cursor : RlpCursor) : SailM (RlpFieldRef × RlpCursor) := do
  ...
  then
    (do
      let full_len ← (( do (byte_quantity_sub next current) ) : SailM byte_length )
...
def rlp_single_ref (item : ByteSlice) : SailM RlpFieldRef := do
...
def rlp_ref_content (f : RlpFieldRef) : SailM ByteSlice := do
  (sub_slice f.source f.content_off f.content_len)

def rlp_ref_full (f : RlpFieldRef) : SailM ByteSlice := do
        ...
        (do
          ...
          then
            ...
            (do
              ...
              let encoded_len ← do (rlp_checked_add_quantity (ByteQuantity (1 +i length_width)) n)
                  ...
                  then
                    ...
                    (do
                      let (len_bytes, length_width) ← do
                        (pure (minimal_word_bytes (← (rlp_length_word n))))
                      let encoded_len ← do
                        (rlp_checked_add_quantity (ByteQuantity (1 +i length_width)) n)
