...
def account_value_changed (value : AcctValue) : Bool :=
  ((! (value.curr.info.nonce == value.orig.info.nonce)) || ((! (value.curr.info.balance == value.orig.info.balance)) || ((! (value.curr.info.storage_root == value.orig.info.storage_root)) || ((! (value.curr.info.code_hash == value.orig.info.code_hash)) || ((! (value.curr.present == value.orig.present)) || (! (value.curr.storage_cleared == value.orig.storage_cleared)))))))

def encode_storage_value (value : (BitVec 256)) : SailM EvmByteSlice := do
...
def encode_state_account (info : AccountInfo) (storage_root : (BitVec 256)) : SailM EvmByteSlice := do
