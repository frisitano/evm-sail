...
def rlp_slice_size (data : EvmByteSlice) : SailM byte_quantity := do
...
def rlp_finish (start : byte_quantity) (expected_len : byte_quantity) : SailM EvmByteSlice := do
...
def rlp_ref_be_length (source : EvmByteSlice) (start : byte_quantity) (count : Nat) : SailM byte_quantity := do
...
def rlp_bytes_equal_at (expected : (List (BitVec 8))) (source : EvmByteSlice) (start : byte_quantity) : SailM Bool := do
...
def rlp_ref_hdr (b : EvmByteSlice) (pos : byte_quantity) (stop : byte_quantity) : SailM (Bool × byte_quantity × byte_quantity) := do
...
/-- Type quantifiers: k_ex159684_ : Bool -/
def rlp_field_ref (source : EvmByteSlice) (is_list : Bool) (full_off : byte_quantity) (full_len : byte_quantity) (content_off : byte_quantity) (content_len : byte_quantity) : RlpFieldRef :=
  { source := source
    is_list := is_list
    full_off := full_off
    full_len := full_len
...
def rlp_ref_fixed_bytes (f : RlpFieldRef) (len : byte_quantity) : SailM EvmByteSlice := do
