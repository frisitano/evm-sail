...
termination_by (let (slots, n, zero, acc, remaining) := (slots, n, zero, acc, remaining)
remaining).toNat
...
def htr_chunk (bytes : EvmByteSlice) (chunk_index : Nat) : SailM (BitVec 256) := do
  let acc : hash := (BitVec.zero 256)
...
/-- Type quantifiers: depth : Nat, 0 ≤ depth ∧ depth ≤ 64 -/
def htr_bytes_root (bytes : EvmByteSlice) (depth : Nat) : SailM (BitVec 256) := do
...
def htr_bytevector (bytes : EvmByteSlice) : SailM (BitVec 256) := do
  let chunks ← do (htr_chunk_count bytes.len)
  (htr_bytes_root bytes (← (clog2 chunks)))

def htr_bytelist (bytes : EvmByteSlice) (limit_bytes : byte_quantity) : SailM (BitVec 256) := do
...
def htr_withdrawal (withdrawal : EvmByteSlice) : SailM (BitVec 256) := do
  (merkleize
...
def htr_versioned_hashes (versioned_hashes : EvmByteSlice) : SailM (BitVec 256) := do
...
def htr_deposit (deposit : EvmByteSlice) : SailM (BitVec 256) := do
  (merkleize
...
def htr_withdrawal_request (request : EvmByteSlice) : SailM (BitVec 256) := do
  (merkleize
...
def htr_consolidation_request (request : EvmByteSlice) : SailM (BitVec 256) := do
  (merkleize
    [(← (htr_bytevector (← (sub_slice request HTR_REQUEST_SOURCE_ADDRESS ADDRESS_BYTE_LENGTH)))), (← (htr_bytevector
...
def htr_deposits (deposits : EvmByteSlice) : SailM (BitVec 256) := do
...
def htr_withdrawal_requests (requests : EvmByteSlice) : SailM (BitVec 256) := do
...
def htr_consolidation_requests (requests : EvmByteSlice) : SailM (BitVec 256) := do
