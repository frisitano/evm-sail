...
/-- Type quantifiers: delta : Nat, 0 ≤ delta -/
def ssz_field_offset (base : byte_quantity) (delta : Nat) : SailM byte_quantity := do
  (byte_quantity_add base (ByteQuantity delta))

def ssz_u32_at (input : EvmByteSlice) (offset : byte_quantity) : SailM Nat := do
...
def ssz_u32 (input : EvmByteSlice) (offset : byte_quantity) : SailM Nat := do
  (ssz_u32_at input offset)

def ssz_limb (input : EvmByteSlice) (offset : byte_quantity) : SailM (BitVec 64) := do
  let b0 ← do (pure (Sail.BitVec.zeroExtend (← (slice_byte input offset)) 64))
  let b1 ← do
    (pure (Sail.BitVec.zeroExtend (← (slice_byte input (← (ssz_field_offset offset 1)))) 64))
  let b2 ← do
...
def ssz_uint (input : EvmByteSlice) (offset : byte_quantity) : SailM Nat := do
  (pure (BitVec.toNatInt (← (ssz_limb input offset))))

def ssz_addr (input : EvmByteSlice) (offset : byte_quantity) : SailM (BitVec 160) := do
  (pure (Sail.BitVec.extractLsb (← (slice_load_n input offset ADDRESS_BYTE_LENGTH)) 159 0))

def ssz_bytes32 (input : EvmByteSlice) (offset : byte_quantity) : SailM (BitVec 256) := do
  (slice_load input offset)

def ssz_logs_bloom (input : EvmByteSlice) (offset : byte_quantity) : SailM (Vector (BitVec 64) 32) := do
  let out : LogsBloom := (vectorInit LIMB_ZERO)
  let cursor : source_pointer := offset
  let (cursor, out) ← (( do
    let loop_k_lower := 0
    let loop_k_upper := 31
...
def ssz_u256 (input : EvmByteSlice) (offset : byte_quantity) : SailM (BitVec 256) := do
