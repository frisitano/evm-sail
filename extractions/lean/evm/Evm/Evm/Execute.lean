...
/-- Type quantifiers: k_ex159820_ : Bool -/
    ...
    (do
      ...
      then
        (mem_mcopy (ByteQuantity (BitVec.toNatInt destination_word))
          (ByteQuantity (BitVec.toNatInt source_word)) (ByteQuantity (BitVec.toNatInt length_word)))
      else (pure ()))
  | .SLOAD () =>
    (do
      let s ← do (pop ())
                                    ...
                                    (do
                                      ...
                                      then
                                        ...
                                        (do
                                          (k_revert checkpoint.state)
                                          (push WORD_ZERO)))))))))))
/-- Type quantifiers: k_ex159824_ : Bool -/
                              ...
                              then (push WORD_ZERO)
                                ...
                                (do
                                  let initcode ← do
                                    if ((← (is_running ())) : Bool)
                                    then
                                      (memory_byte_slice (ByteQuantity (BitVec.toNatInt off_word))
                                        (ByteQuantity initcode_size))
                                    else (pure EMPTY_SLICE)
                                  let child_code ← do
