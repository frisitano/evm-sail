...
def decode_single_stack_index (immediate : byte) : SailM deep_stack_index := do
  let publicResult ← do
...
def decode_exchange_stack_indices (immediate : byte) : SailM (stack_index × stack_index) := do
  let publicResult ← do
