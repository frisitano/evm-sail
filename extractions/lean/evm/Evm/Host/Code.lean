...
partial def analyze_code_from (code : EvmByteSlice) (table : (BitVec 64)) (pc : byte_quantity) (analysis : CodeAnalysis) : SailM Unit := do
...
termination_by (let (code, table, pc, analysis) := (code, table, pc, analysis)
let .ByteQuantity code_len := code.len
let .ByteQuantity position := pc
(code_len -i position)).toNat

def analyze_code (code : EvmByteSlice) : SailM (BitVec 64) := do
...
def code_db_insert (code : EvmByteSlice) : SailM (BitVec 256) := do
