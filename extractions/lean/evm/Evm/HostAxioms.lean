...
def mem_expand (required : source_length) : SailM (EvmByteSliceLength required) := do
...
def host_slice_count_nonzero (slice : EvmByteSlice) : SailM host_access := do
  pure (materializeSlice (← get) slice |>.countP (· != 0))

def host_slice_strided_zero
    (slice : EvmByteSlice) (start stride width count : host_access) : SailM Bool := do
  let state ← get
...
def host_slice_load_n_word
    (slice : EvmByteSlice) (position : source_pointer) (count : host_access) :
    SailM word := do
  let state ← get
...
def host_slice_copy_to_memory
    (slice : EvmByteSlice) (dst : memory_pointer)
...
def bytes_segments_equal_slice
    (segments : List Bytes) (slice : EvmByteSlice) : SailM Bool := do
  let state ← get
...
private def acceleratorOutput
    (selector : UInt8) (slice : EvmByteSlice)
...
private def acceleratorCheck
    (selector : UInt8) (slice : EvmByteSlice) : SailM Bool := do
  let state ← get
...
private def acceleratorPairing
    (selector : UInt8) (slice : EvmByteSlice) : SailM (BitVec 2) := do
  let state ← get
  pure <| match callNative selector (materializeSlice state slice) 0 0 0 with
    | some output =>
...
def accelerator_ripemd160 (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 2 slice 0 0 0

def accelerator_modexp
    (slice : EvmByteSlice) (base exponent modulus : Nat) : SailM Bool :=
  acceleratorOutput 3 slice base exponent modulus

def accelerator_bn254_add (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 4 slice 0 0 0

def accelerator_bn254_mul (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 5 slice 0 0 0

def accelerator_bn254_pairing (slice : EvmByteSlice) : SailM (BitVec 2) :=
...
def accelerator_bls_g1_add (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 9 slice 0 0 0

def accelerator_bls_g1_msm (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 10 slice 0 0 0

def accelerator_bls_g2_add (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 11 slice 0 0 0

def accelerator_bls_g2_msm (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 12 slice 0 0 0

def accelerator_bls_pairing (slice : EvmByteSlice) : SailM (BitVec 2) :=
  acceleratorPairing 13 slice

def accelerator_bls_map_fp_to_g1 (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 14 slice 0 0 0

def accelerator_bls_map_fp2_to_g2 (slice : EvmByteSlice) : SailM Bool :=
  acceleratorOutput 15 slice 0 0 0

def accelerator_p256_verify (slice : EvmByteSlice) : SailM Bool :=
...
def output_buffer_store (slice : EvmByteSlice) : SailM Bool := do
...
def public_output_write (slice : EvmByteSlice) : SailM Bool := do
  let bytes := materializeSlice (← get) slice
