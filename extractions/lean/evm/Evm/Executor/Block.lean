...
def execute_block_transactions (transactions : TransactionListRef) (public_keys : EvmByteSlice) (header_gas_limit : block_gas_limit) : SailM BlockExecutionResult := do
  ...
  let public_key_count_value := (Int.ediv public_keys_length public_key_length)
  ...
  let keys : (Sigma fun (k_off : Nat) => (Sigma fun (k_len : Nat) => (EvmByteSliceFields k_off k_len))) :=
    ((⟨_, ⟨_, public_keys⟩⟩ : (Sigma fun (k_off : Nat) =>
  ...
  let (blob_gas_acc, cursor, execution_gas_acc, keys, receipts, state_gas_acc, tx0_to) ← (( do
    let loop_vars ← whileFuelM (fuel :=(cursor.items.count -i cursor.index)) (fun (blob_gas_acc, cursor, execution_gas_acc, keys, receipts, state_gas_acc, tx0_to) => (pure (! (ssz_list_cursor_empty
      ...
      fun (blob_gas_acc, cursor, execution_gas_acc, keys, receipts, state_gas_acc, tx0_to) => do
        ...
        if ((keys_length <b public_key_length) : Bool)
        then sailThrow ((InvalidBlock WitnessDeficient))
        else (pure ())
        let public_key := (sub_slice keys_fields 0 PUBLIC_KEY_LENGTH)
        let keys : (Sigma fun (k_off : Nat) =>
          (Sigma fun (k_len : Nat) => (EvmByteSliceFields k_off k_len))) :=
        ...
        let (blob_gas_acc, execution_gas_acc, receipts, state_gas_acc) ← (( do
          ...
          then sailThrow ((InvalidBlock GasUsedExceedsLimit))
            ...
            (do
              let tx_blob_gas ← do
                (do
                    let publicResult ← (transaction_blob_gas_for_count
              ...
              let next_blob_gas ← (( do
                ...
                then (pure blob_gas_acc)
                  ...
                  (do
                      let publicResult ← (checked_block_blob_gas_add ⟨blob_gas_acc⟩
                      ⟨tx_blob_gas⟩)
              ...
              (pure (blob_gas_acc, execution_gas_acc, receipts, state_gas_acc))) ) : SailM
          (Nat × Nat × ReceiptAccumulator × Nat) )
        (pure ((blob_gas_acc, cursor, execution_gas_acc, keys, receipts, state_gas_acc, tx0_to) : (Nat × (BoundedSszListCursor (2 ^ 20)) × Nat × (Sigma
          fun (k_off : Nat) => (Sigma fun (k_len : Nat) => (EvmByteSliceFields k_off k_len))) × ReceiptAccumulator × Nat × (Vector (BitVec 8) 20))))
    (pure loop_vars) ) : SailM
    (Nat × (BoundedSszListCursor (2 ^ 20)) × Nat × (Sigma fun (k_off : Nat) =>
    (Sigma fun (k_len : Nat) => (EvmByteSliceFields k_off k_len))) × ReceiptAccumulator × Nat × (Vector (BitVec 8) 20))
    )
...
def apply_withdrawals (withdrawals : WithdrawalListRef) : SailM Unit := do
  ...
  let rest ← (( do
    let loop_vars ← whileFuelM (fuel :=rest.count) (fun rest => (pure (rest.count != 0))) rest
      fun rest => do
        ...
        let withdrawal ← do
          (decode_withdrawal
            (⟨_, ⟨_, ((withdrawal_ref).2).2⟩⟩ : (Sigma fun (k_off : Nat) =>
            (Sigma fun (k_len : Nat) => (EvmByteSliceFields k_off k_len)))))
        (k_add_balance withdrawal.address
          (alu_mul ((word_of_withdrawal_amount ⟨(withdrawal.amount).value⟩)).value 1000000000))
        (pure rest)
...
def execute_block_body (body : BlockBody) (public_keys : EvmByteSlice) (header_gas_limit : block_gas_limit) : SailM BlockExecutionResult := do
  ...
  let result ← do
    (execute_block_transactions body.transactions ⟨_, ⟨_, public_keys⟩⟩
      ⟨header_gas_limit⟩)
