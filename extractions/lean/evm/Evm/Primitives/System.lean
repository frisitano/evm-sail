...
def SYSTEM_ADDRESS : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE#256))).value⟩)

def BEACON_ROOTS_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x000000000000000000000000000F3DF6D732807EF1319FB7B8BB8522D0BEAC02#256))).value⟩)

def HISTORY_STORAGE_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x0000000000000000000000000000F90827F1C53A10CB7A02335B175320002935#256))).value⟩)

def WITHDRAWAL_REQUEST_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x00000000000000000000000000000961EF480EB55E80D19AD83579A64C007002#256))).value⟩)

def CONSOLIDATION_REQUEST_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x0000000000000000000000000000BBDDC7CE488642FB579F8B00F3A590007251#256))).value⟩)

def BUILDER_DEPOSIT_REQUEST_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x0000000000000000000000000000BFF46984E3725691FA540A8C7589300D8282#256))).value⟩)

def BUILDER_EXIT_REQUEST_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x000000000000000000000000000064D678505AD48F8CCB093BC65613800E8282#256))).value⟩)

def DEPOSIT_CONTRACT_ADDR : address :=
  (word_to_address
    ⟨((U256
      (BitVec.toNatInt 0x00000000000000000000000000000000219AB540356CBB839CBE05303D7705FA#256))).value⟩)

def DEPOSIT_EVENT_TOPIC : word :=
  ⟨((U256 (BitVec.toNatInt 0x649BBC62D0E31342AFEA4E5CD82D4049E7E1EE912FC0889AA790803BE39038C5#256))).value⟩
...
def EIP7708_TRANSFER_TOPIC : word :=
  ⟨((U256 (BitVec.toNatInt 0xDDF252AD1BE2C89B69C2B068FC378DAA952BA7F163C4A11628F55A4DF523B3EF#256))).value⟩

def EIP7708_BURN_TOPIC : word :=
  ⟨((U256 (BitVec.toNatInt 0xCC16F5DBB4873280815C1EE09DBD06736CFFCC184412CF7A71A0FDB75D397CA5#256))).value⟩
